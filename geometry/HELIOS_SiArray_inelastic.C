...
const TString geoVersion = "HELIOS_SiArray_inelastic";
...
const Float_t Recoil_Z_dist_from_target     = 40.0;// Distance of the recoil detector with respect to the target
const Float_t Si_width                      = 1.0;// Width of Si detectors
const Float_t Array_length                  = 38.0;// 
const float PCB_thickness                   = 0.5;//cm  
float blocker_length                        = 10.0;//                                                        
const Float_t Z_dist_from_target            = Recoil_Z_dist_from_target+PCB_thickness+blocker_length+Array_length-4.0;//cm (if positive, the length of the array/2.0 (38) has to be added) 50+38/2+5/2(recoil det thickness)
...
TGeoVolume* create_detector()
{
  ...
  gGeoMan->GetVolume(geoVersion)->AddNode(siliconYZL_5,35, new TGeoTranslation(y_off/2,0,-29.487+Z_dist_from_target));
  siliconYZL_5->SetLineColor(kGray);
 
  TGeoVolume *PCBYZL = gGeoManager->MakeBox("PCBYZL",pcb,  0.1/2, 2.0/2, Array_length/2);
  gGeoMan->GetVolume(geoVersion)->AddNode(PCBYZL,7, new TGeoTranslation((y_off-0.17)/2,0,-15.0+Z_dist_from_target));
  ...
  gGeoMan->GetVolume(geoVersion)->AddNode(siliconYZR_5,36, new TGeoTranslation(-y_off/2,0,-29.487+Z_dist_from_target));
  siliconYZR_5->SetLineColor(kGray);
 
  TGeoVolume *PCBYZR = gGeoManager->MakeBox("PCBYZR",pcb,  0.1/2, 2.0/2, Array_length/2);
  gGeoMan->GetVolume(geoVersion)->AddNode(PCBYZR,13, new TGeoTranslation(-(y_off-0.17)/2,0,-15.00+Z_dist_from_target));
  ...
  gGeoMan->GetVolume(geoVersion)->AddNode(siliconXZT_5,37, new TGeoCombiTrans(0,x_off/2.0,-29.487+Z_dist_from_target,rTop));
  siliconXZT_5->SetLineColor(kGray);

  TGeoVolume *PCBXZT = gGeoManager->MakeBox("PCBXZT",pcb,  0.1/2, 2.0/2, Array_length/2);
  gGeoMan->GetVolume(geoVersion)->AddNode(PCBXZT,19, new TGeoCombiTrans(0,(x_off-0.17)/2,-15.00+Z_dist_from_target,rTop));
  ...
  gGeoMan->GetVolume(geoVersion)->AddNode(siliconXZB_4,38, new TGeoCombiTrans(0,-x_off/2.0,-29.487+Z_dist_from_target,rBottom));
  siliconXZB_5->SetLineColor(kGray);

  TGeoVolume *PCBXZB = gGeoManager->MakeBox("PCBXZB",pcb,  0.1/2, 2.0/2, Array_length/2);
  gGeoMan->GetVolume(geoVersion)->AddNode(PCBXZB,25, new TGeoCombiTrans(0,-(x_off-0.17)/2,-15.00+Z_dist_from_target,rBottom));
  ...
  //Target
  TGeoVolume *target = gGeoManager->MakeBox("siliconTarget", cd2,  4.0/2, 4.0/2, 0.000110/2);
  gGeoMan->GetVolume(geoVersion)->AddNode(target,32, new TGeoTranslation(0.0,0.0,0.0));
  target->SetLineColor(kGray+1);

  //Blocker
  TGeoVolume *blocker = gGeoManager->MakeTubs("blocker", cd2,  1.5/2, 1.8/2, blocker_length/2,0,360);
  gGeoMan->GetVolume(geoVersion)->AddNode(blocker,35, new TGeoCombiTrans(0,0,(Recoil_Z_dist_from_target+PCB_thickness/2.0+blocker_length/2.0),new TGeoRotation("rblocker",0,0,0)));
  blocker->SetLineColor(kRed);
