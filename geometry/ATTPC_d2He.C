...
const TString geoVersion = "ATTPC_d2He_07atm";
...
const Float_t fc_inner_diameter = 50.;
const Float_t fc_outer_diameter = 50.5;
const Float_t fc_drift_length = 100.;

// Chamber
const Float_t chamber_diameter = 62.;
const Float_t chamber_length = 120.;
const Float_t chamber_thickness = 61.; // 62 cm - 1 cm of thickness

//FC rings
const Float_t fc_inner_ring_idiam = 0.0;
const Float_t fc_inner_ring_odiam = 0.5;
const Float_t fc_inner_ring_diam = 46.0;

const Float_t fc_outer_ring_idiam = 0.0;
const Float_t fc_outer_ring_odiam = 0.5;
const Float_t fc_outer_ring_diam = 49.5;
...
void create_materials_from_media_file()
{
  ...
  FairGeoMedium* TargetD2          = geoMedia->getMedium("TargetD2_07");
...
TGeoVolume* create_detector()
{
  ...
  TGeoVolume *fc_volume = gGeoManager->MakeTube("fc_volume",fc,fc_inner_diameter/2,fc_outer_diameter/2,fc_drift_length/2);
  ...
  TGeoVolume *chamber_volume = gGeoManager->MakeTube("chamber_volume", OuterCylinder,chamber_thickness/2, chamber_diameter/2, chamber_length/2);
  gGeoMan->GetVolume(geoVersion)->AddNode(chamber_volume,1, new TGeoTranslation(0,0,chamber_length/4));
    
  TGeoVolume *fc_inner_ring = gGeoManager->MakeTorus("fc_inner_ring", fc_rings_med,fc_inner_ring_diam/2.0,fc_inner_ring_idiam/2.0,fc_inner_ring_odiam/2.0);
