...
void geant4_fixed(Double_t thetaCms = 45.0, Int_t nEvents = 100, UInt_t seed = 42, TString mcEngine = "TGeant4")
{
   ...
   TString outFile = "./data/geant4_fixed.root";
   TString parFile = "./data/geant4_fixed_params.root";
   ...
   auto *primGen = BuildElasticGenerator(thetaCms, thetaCms);
   run->SetGenerator(primGen);

   run->SetStoreTraj(kFALSE);
   LOG(INFO) << "Initializing geometry and field. Starting simulation with " << nEvents
             << " events and seed " << seed;
   run->Init();
   LOG(INFO) << "------ Finished intialization ------";
   ...
   LOG(INFO) << "Starting simulation with " << nEvents << " events and seed " << seed;
   run->Run(nEvents);
