...
def Analyze_H5(h5_dir, file, threshold, ADD_NOISE=False):
    ...
    def main(h5file, threshold, ADD_NOISE):
        ...
        for i in range(0, num_events):
            ...
            # Apply pad threshold
            # Replace the line event = np.array(h5file[str_event][:]) with the following code
            event_dataset = h5file[str(str_event)]  # Convert str_event to a string
            event = np.array(event_dataset['HitArray'])  
          ...
          #  str_trace = f"Trace_[{i}]"
            trace = np.array([h5file[str(str_event)]['Trace']])
...
try:
    output_df = pd.read_csv(h5_directory + 'numerical.csv')
...
for file in os.listdir(h5_directory):
    if file.endswith(".h5") and file not in output_df['file'].values:
        print(file)
        output_df = output_df.append(Analyze_H5(h5_directory, file, ADD_NOISE=False, threshold=5))
