...
void runana_digi_d2He()
{
	...
	std::string digiFileName = "../outputFiles/attpcdigi_d2He_10.root";
	...
	TFile* outfile;
	TString  outFileNameHead = "../outputFiles/attpcanadigi_10.root";//attpcsim_d2He_test20k
	...
	TH2D *ata_dta = new TH2D ("ata_dta", "ata_vs_dta", 100, -0.1, 0.1, 100, -0.1, 0.1);
	...
	anatree->Branch("ivt_same",&ivt_same);
	...
	anatree->Branch("kin_He2_same",&kin_He2_same);
	anatree->Branch("theta_He2_same",&theta_He2_same);
	anatree->Branch("Ex_reco_same",&Ex_reco_same);
	...
	Int_t N = 52;
	std::vector < Double_t > CoeffCube (N), CoeffSqr (N), CoeffLin (N), CoeffConst (N);
	std::vector < Double_t > Knots (N+1);
	Double_t col1, col2, col3, col4, row1;
...
/// --------------------- Event loop -------------------------------------------
	...
	for(Int_t i=0;i<nEvents;i++){
		...
		if(fATRansac==NULL) std::cout<<" Null pointer fATRansac "<<"\n";
			...
			// std::cout<< std::endl << "###### Event number " << tree->GetReadEvent() << " ###### Number of hits " << nHits << "\n";
			...
			if(i%2!=0){
				...
				for(Int_t iHit=0; iHit<nHits; iHit++){
					...
					// if(pow(pow(hitPos.X(),2)+pow(hitPos.Y(),2),0.5)>50.){
							tracks_z_r->Fill (1000.0 - hitPos.Z(), pow(pow(hitPos.X(),2)+pow(hitPos.Y(),2),0.5));
							tracks_x_y->Fill (hitPos.X(), hitPos.Y());
					//}

				}// Hit loop

				std::vector<ATRANSACN::ATRansac::PairedLines> lines = fATRansac->GetPairedLinesArray();//GetLines_ana

				if(lines.size()>0){

					if(lines.at(0).MaxR.first>=242. || lines.at(0).MaxR.second>=242. || lines.at(0).MaxR.first<=25 || lines.at(0).MaxR.second<=25 || lines.at(0).LastPoint.first.Z()>973. || lines.at(0).LastPoint.second.Z()>973.) continue; //if do no stop in chamber or in the beam hole

					lastX1 = lines.at(0).LastPoint.first.X();
					lastY1 = lines.at(0).LastPoint.first.Y();
					lastZ1 = lines.at(0).LastPoint.first.Z();
					lastX2 = lines.at(0).LastPoint.second.X();
					lastY2 = lines.at(0).LastPoint.second.Y();
					lastZ2 = lines.at(0).LastPoint.second.Z();
					angle12 = lines.at(0).angle;
					vertexX = lines.at(0).meanVertex.X();
					vertexY = lines.at(0).meanVertex.Y();
					vertexZ = lines.at(0).meanVertex.Z();

					theta1 = lines.at(0).AngTheta.first* TMath::DegToRad();
					theta2 = lines.at(0).AngTheta.second* TMath::DegToRad();
					phi1 = lines.at(0).AngPhi.first* TMath::DegToRad();
					phi2 = lines.at(0).AngPhi.second* TMath::DegToRad();
					//std::cout<<i<<" protons 1 2 theta : "<<theta1<<" "<<theta2<<"\n";
					//std::cout<<i<<" protons 1 2 phi : "<<phi1<<" "<<phi2<<"\n";

					range_p1 = sqrt(pow(vertexZ-lastZ1,2) + pow(vertexX-lastX1,2)+ pow(vertexY-lastY1,2));
					range_p2 = sqrt(pow(vertexZ-lastZ2,2) + pow(vertexX-lastX2,2)+ pow(vertexY-lastY2,2));
...
//==============================================================================
					...
					epsilon_pp = 0.5*(eLoss_p1_reco + eLoss_p2_reco - 2 * sqrt(eLoss_p1_reco * eLoss_p2_reco) * TMath::Cos (lines.at(0).angle));
					epsilon_pp_reco->Fill(epsilon_pp);

				//	if(epsilon_pp<0.2) std::cout<<eLoss_p1_reco<<" "<<eLoss_p2_reco<<" "<<TMath::Cos (lines.at(0).angle)<<std::endl;
					...
					//add conditions on the eLoss?...
					mom_He2_reco = mom_proton1_reco + mom_proton2_reco;
					// std::cout<<" mom reco angle 1  "<<mom_proton1_reco.Theta()<<" "<<mom1_norm_reco<<" "<<" "<<std::endl;
					// std::cout<<" mom reco angle 2  "<<mom_proton2_reco.Theta()<<" "<<mom2_norm_reco<<std::endl;
					// std::cout<<" mom reco angle tot "<<mom_He2_reco.Theta()<<std::endl;
...
//to compare with juan sim file result (semi digi and no randsac)
/*
					cout<<"  "<<endl;
					cout<<"Event "<<i<<endl;
					cout<<"Vertex "<<vertexZ<<" "<<vertexX<<" "<<vertexY<<endl;
					cout<<"2 pos // range "<<lastZ2<<" "<<lastX2<<" "<<lastY2<<" // "<<range_p2<<" "<<endl;
					cout<<"1 pos // range "<<lastZ1<<" "<<lastX1<<" "<<lastY1<<" // "<<range_p1<<" "<<endl;
					cout<<"Eloss "<<eLoss_p2_reco<<" "<<eLoss_p1_reco<<endl;
					cout<<"Angles theta // phi "<<theta2* TMath::RadToDeg()<<" "<<theta1* TMath::RadToDeg()<<" // "<<phi2* TMath::RadToDeg()<<" "<<phi1* TMath::RadToDeg()<<endl;
