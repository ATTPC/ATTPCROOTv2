...
void d2He_ana_1atm()
{
	...
	TFile* outfile;
        //outfile   = TFile::Open("hist_d2He_ana_12C.root","recreate");
	//outfile   = TFile::Open("histd2He_6Li_hole40_1atm.root","recreate");
	...
	Double_t mass_proj_uma = 6.01512288741;
	Double_t mass_reco_uma = 6.018886648;
	Double_t Ekin_proj = 600.0;
	...
	Double_t P2[3];
		...
		//cout <<energyLoss_p1<<"  "<<range_p1<<"  "<<eLoss_p1_hi<<"  "<<range1_hi<<"  "<<(GetEloss(eLoss_p1_hi, range1_hi, sqrt( sq(xpos1_hi) + sq(ypos1_hi) + sq(zpos1_hi - real_vertex)), X, Y)+eLoss_p1_hi-energyLoss_p1)/energyLoss_p1*100<<endl;	  

		//cout <<energyLoss_p1<<"  "<<range_p1<<"  "<<eLoss_p1_hi<<"  "<<range1_hi<<"  "<<GetEloss(eLoss_p1_hi, range1_hi, sqrt( sq(xpos1_hi) + sq(ypos1_hi) + sq(zpos1_hi - real_vertex)))<<endl;	  
		...
		ereal_thres->Fill(realkinE, av_dE_p1);
		   ...
		   //cout<<guess_x1_slope<<" "<<P0[2]<<" "<<guess_z1_slope<<" "<<endl;
	           double pStart[4] = {0.2,guess_x1_slope,P0[2],guess_z1_slope};
		   ...
		   // get fit parameters
   		   const double * parFit = result.GetParams();  
                   ...
                   TFitResultPtr  res1 = grphi1->Fit("pol1","Sq"); 
		    ...
		    //delete fitter;
                  ...
                  //std::vector<Double_t> unvector (6);
                  //unvector = {0.2,guess_x1_slope,P0[2],guess_z1_slope,lastX1, lastZ1};

                  //d2heana->FitTrack( gr1 , grphi1, &unvector );

                    //cout<<theta_p1_fit<<" "<<phi_p1_fit<<" "<<parFit[2]<<" "<<ok<<endl;
                    //cout<<d2heana->GetThetaFit()<<" "<<d2heana->GetPhiFit()<<" "<<d2heana->GetVertexFit()<<" "<<d2heana->GetFitStatus()<<endl;
                    //cout<<endl;
		   ...
		   //cout<<guess_x2_slope<<" "<<P0[2]<<" "<<guess_z2_slope<<" "<<endl;
   		   double pStart2[4] = {0.2,guess_x2_slope,P0[2],guess_z2_slope};
   		   fitter2.SetFCN(fcn2,pStart2);
		   
		   fitter2.Config().ParSettings(0).SetLimits(-10.0,10.0);
		   fitter2.Config().ParSettings(1).SetLimits(-20,20);
		   fitter2.Config().ParSettings(2).SetLimits(-10,1000);
		   fitter2.Config().ParSettings(3).SetLimits(-20,20);
		   
   		   // set step sizes different than default ones (0.3 times parameter values)
   		   for (int i = 0; i < 4; ++i) fitter2.Config().ParSettings(i).SetStepSize(0.01);
		   ...
		   // get fit parameters
   		   const double * parFit2 = result2.GetParams();  
                   ...
                   TFitResultPtr  res2 = grphi2->Fit("pol1","Sq"); 
		   ...
		   theta_res->Fill(avetheta_p2,theta_p2_fit);
                   ...
                   //cout<<parFit[2]<<" "<<parFit2[2]<<" "<<avetheta_p2<<" "<<theta_p2_fit<<endl;

		

                bool goodfit = false;
                if(ok==true && ok2==true &&  (fabs(parFit2[2] -parFit[2])< 30)  ) goodfit = true;
		...
		//cout<<eLoss_p1_reco<<"  "<<energyLoss_p1<<"  "<<range1_low<<"  "<<range1_hi<<"  "<<eLoss_p1_hi<<endl;
       ...
       double *kinematics_res = kine_2b(proj_mass, target_mass,  he2_mass_ex,  recoil_mass, Ekin_proj, theta_He2*TMath::DegToRad(), kin_He2);
		...
		//cout<<"Fitted angles*****Event: "<<iEvent<<endl;
		//cout<<theta_p1_fit<<"  "<<theta_p2_fit<<"  "<<mom_He2_res.Mag()<<"  "<<he2_mass_ex- he2_mass<<endl;
       ...
       double *kinematics = kine_2b(proj_mass, target_mass,  he2_mass_ex,  recoil_mass, Ekin_proj, theta_He2*TMath::DegToRad(), kin_He2);
		...
		//cout<<"Real angles*******"<<endl;
                //cout<<avetheta_p1<<"  "<<avetheta_p2<<"  "<<mom_He2.Mag()<<"  "<<he2_mass_ex- he2_mass<<endl;
		//cout<<endl;
