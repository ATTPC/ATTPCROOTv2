		...
		//for(Int_t i=0;i<10;i++){
	     ...
	     Bool_t gated = cevent->IsExtGate();
			...
			*/
        	...
        	theta1 = trackCand.at(two_p_pair[w].first).GetThetaPhi(vertexMean, lastPoint1,-1).first;
        	theta2 = trackCand.at(two_p_pair[w].second).GetThetaPhi(vertexMean, lastPoint2,-1).first;
        	phi1 = trackCand.at(two_p_pair[w].first).GetThetaPhi(vertexMean, lastPoint1,-1).second;
        	phi2 = trackCand.at(two_p_pair[w].second).GetThetaPhi(vertexMean, lastPoint2,-1).second;
          ...
          // reconstruction of 2He
        	...
        	mom_proton1_reco.SetZ (mom1_norm_reco * TMath::Cos(theta1));
					Double_t Ep1Tot = eLoss_p1_reco + proton_mass;
        	...
        	mom_proton2_reco.SetZ (mom2_norm_reco * TMath::Cos(theta2));
					Double_t Ep2Tot = eLoss_p2_reco + proton_mass;
        	...
        	//std::cout<<i<<" mom2 : "<<mom_proton2_reco.Mag()<<"\n";
					...
					if(i%2==0) continue;
						...
						alphain.push_back(Pejec.X());
						alphain.push_back(Pejec.Y());
						alphain.push_back(Pejec.Z());
						alphain.push_back(EejeTot);
						...
						d2heana->KFit(alphain,alphaout);
						mom_proton1_reco.SetXYZ(alphaout[8],alphaout[9],alphaout[10]);
						Ep1Tot = alphaout[11];
						mom_proton2_reco.SetXYZ(alphaout[12],alphaout[13],alphaout[14]);
						Ep2Tot = alphaout[15];
						alphain.clear();
						alphaout.clear();
