void Mg22He4_sim(Int_t nEvents = 10000, TString mcEngine = "TGeant4")
{
   ...
   Pyp.push_back(py);
   Pzp.push_back(pz);
   Mass.push_back(21.999573843); // uma
   ...
   zB = 12; // 22Mg
   aB = 22;
   massDecayB = 21.999573843;
   massTarget = 0.0;
   exEnergy = 0.0; // NB: Set to zero for sequential decay

   // Heavy residue first
   zDecay.at(0).push_back(10); // 20Ne
   aDecay.at(0).push_back(20);
   qDecay.at(0).push_back(0);
   massDecay.at(0).push_back(19.99244017542);

   for (auto i = 0; i < 2; ++i) { // 2 protons
      zDecay.at(0).push_back(1);
      aDecay.at(0).push_back(1);
      qDecay.at(0).push_back(0);
      massDecay.at(0).push_back(1.00727646);
   }
