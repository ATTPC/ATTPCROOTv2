...
void run_sim_ana(Int_t num_ev=100000)
{
    ...
    TH2D *HKineRecoil =  new TH2D("HKineRecoil","HKineRecoil",1000,0,180,1000,0,20);
    TH2D *AngRange[10];
	...
	rad->Reset();
        ...
        for(Int_t i=0; i<n; i++) {
            ...
            pointMC = (AtMCPoint*) pointMCArray ->At(i);
	...
	//	zpos=point->GetZ();
	 ...
	 //   }
		...
		n2++;
                ...
                Double_t A = ((EnergyRecoil/4.0 + 931.5) * (EnergyRecoil/4.0 + 931.5) - 931.5 * 931.5)/(2 * 931.5);
		...
		if(AngleRecoil>65 && AngleRecoil<70 && vertex>200.0 && vertex<210.0 && zpos>200.0 ){
			...
			//Radius of curvature calculation
				...
				if(i<5){
				 nrad++;
				 point_forw = (AtMCPoint*) pointArray -> At(i+1);
				 point_back = (AtMCPoint*) pointArray -> At(i+2);
				 Double_t x1 = point->GetX()*10;
            			 Double_t x2 = point_forw->GetX()*10;
           		         Double_t x3 = point_back->GetX()*10;
           		         Double_t y1 = point->GetY()*10;
        		         Double_t y2 = point_forw->GetY()*10;
          		         Double_t y3 = point_back->GetY()*10;
				 Double_t z2 = point_forw->GetZ()*10;
	...
	//std::cout<<" Mean radius of curvature (hist) : "<<rad->GetMean()<<std::endl;
        ...
        //if(iEvent%2!=0){
           ...
           // std::cout<<" energyLoss_sca : "<<energyLoss_sca<<std::endl;
            ...
            HKineRecoil->Fill(AngleRecoil,Bro);
