...
void Li11pp_ana(Int_t num_ev=20000)
{
	  ...
	  /*  TCanvas *c3 = new TCanvas();
	    ...
	    //std::vector<TString> files = {"data/attpcsim_35P_aa_gs","data/attpcsim_35P_aa_3_2+","data/attpcsim_35P_aa_1_2+2","data/attpcsim_35P_aa_5_2+"};
            std::vector<TString> files = {"data/attpcsim"};
            //std::vector<double> ex_energy = {0.0,2.386,3.0,3.859};
            std::vector<double> ex_energy = {0.75};
            //std::vector<int> xs_events = {20000,20000,2000,20000};
            std::vector<int> xs_events = {20000};
          ...
          for(auto iFile=0;iFile<files.size();++iFile)
	  {
				...
				if(iEvent%100==0) std::cout<<" Event Number : "<<iEvent<<std::endl;
				rad->Reset();
				...
				if(iEvent%2!=0){
				   ...
				   // std::cout<<" energyLoss_sca : "<<energyLoss_sca<<std::endl;
				    ...
				    //Resolution
				    Double_t AngleRecoil_res = Gauss(AngleRecoil,0.500);
				    Double_t EnergyRecoil_res = Gauss(EnergyRecoil,EnergyRecoil*0.05);

				    //Q-value calculation
				    Double_t m_a   = 1.007825*931.49401;
				    Double_t m_P35 = 11.043723*931.49401;//35P
				    Double_t m_beam = 11.043723*931.49401;
                                    Ebeam_buff = (EnergyRecoil + EnergySca + ex_energy[iFile]);
		Double_t Qval = EnergyRecoil*(1 + m_a/m_P35) - Ebeam_buff*(1 - m_beam/m_P35) - (2.0/m_P35)*TMath::Sqrt(Ebeam_buff*EnergyRecoil_res*m_a)*TMath::Cos(AngleRecoil_res);

                                    //Excitation energy
				    Double_t ex_energy_exp = kine_2b(m_P35,m_a,m_a,m_P35,Ebeam_buff,TMath::DegToRad()*AngleRecoil_res,EnergyRecoil_res);
                                    //in this definition: m1(projectile); m2(target); m3(ejectile); and m4(recoil);

				    //std::cout<<" Beam energy "<<Ebeam_buff<<" Qval "<<Qval<<"\n";


				    if(vertex_buff<1000 && vertex_buff>0){
					 HKineRecoil->Fill(AngleRecoil_res,EnergyRecoil_res);
					 if(EnergyRecoil<6.0 && iEvent<xs_events[iFile]) HQval->Fill(ex_energy_exp);
					 Kine->SetPoint(Kine->GetN(),AngleRecoil,EnergyRecoil);
				    }
