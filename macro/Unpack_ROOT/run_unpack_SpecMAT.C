...
void run_unpack_SpecMAT(TString dataFile = "./data/TTreesGETrun_9993.root",
  ...
  if (!check_file(dataFile.Data())) {
    std::cout << cRED << " Run file " << dataFile.Data()
              << " not found! Terminating..." << cNORMAL << std::endl;
    exit(0);
  }
  ...
  if (fUseSeparatedData)
    fDecoderTask->SetPseudoTopologyFrame(
        kTRUE); //! This calls the method 10 times so for less than 10 CoBos
                //! ATCore2 must be modified
  Bool_t IsCoboPositivePolarity[4] = {
      true, false, true,
