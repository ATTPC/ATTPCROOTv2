...
TRelativisticKinematics::TRelativisticKinematics() {
	...
	void TRelativisticKinematics::Kinematics() {
		...
		double e_cm2 = e*e-pb2; // Total energy of CM is just the mass (Lorentz Invariant M^2=(E1+E2)-(p1+p2)), therefore we subtract the momentum to the total energy P=(M,0)
		...
		double tg_thetalabs=p3_cm*sin(thetacms)/(gamma*(p3_cm*cos(thetacms)+beta*sqrt(p3_cm*p3_cm+wm3*wm3)));
		...
		double tg_thetalabr=p4_cm*sin(thetacmr)/(gamma*(p4_cm*cos(thetacmr)+beta*sqrt(p4_cm*p4_cm+wm4*wm4)));
