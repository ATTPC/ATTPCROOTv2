...
void kine_ana_transfer_reunpack(int runNum)
{
   ...
   TString output_filename = TString::Format("./transfer_data/simple_%d_test.root", runNum);
   ...
   tree_new->Branch("dE", dE, "dE[track_num]/D");
   tree_new->Branch("E", E, "E[track_num]/D");
	...
	// continue;
      ...
      for (auto &track: tracks){
	    ...
	    if (sizeID == 1) {
  	     ...
  	     if(currentRangeInSmallPads > 0 && currentRangeInSmallPads < 100)dE[track_num] = smallPadCharge;
	    
	     if (currentRangeInSmallPads > rangeInSmallPads)
               rangeInSmallPads = currentRangeInSmallPads;
         }
	 
	 E[track_num] = (smallPadCharge - dE[track_num])+ bigPadCharge;
