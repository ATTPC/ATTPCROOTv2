...
void SetERtable(){//fit of the GEANT4 E vs R obtained from the simulation with the function model given by LISE++
	...
	//	ifstream fER("p_in_d_350torr.txt");//from LISE++,
		...
		ifstream fER("eLossTables/he_in_he.txt");
		...
		for (string line; getline(fER, line);) {
			...
			iRE.push_back(l1*4.0);//E in MeV/u to MeV
			iRE.push_back(l2/1000.);//range in um to mm, model 1
			iRE.push_back(l3/1000.);//range in um to mm, model 2
			iRE.push_back(l4/1000.);//range in um to mm, model 3
			iRE.push_back(l5/1000.);//range in um to mm, model 4
...
void analysis(int runNumberS800, int runNumberATTPC)
	{
		...
		//std::string digiFileName = "run_2101_0101.root";//merged.root
		...
		TString outFileNameHead = TString::Format("/mnt/analysis/e18027/rootAna/runAnalyzed_%04d_%04d.root", runNumberS800, runNumberATTPC);
    ...
    //std::cout<<" "<<S800_timeObjSelect<<" "<<XfObj_tof<<" "<<S800_ICSum<<std::endl;
			...
			//std::cout<<i<<" "<<" "<<S800_timeStamp<<" "<<S800_timeRf<<" "<<S800_y0<<std::endl;
	        	...
	        	theta1 = trackCand.at(w).GetThetaPhi(vertexMean, lastPoint1,1).first;//GetThetaPhi(..,..,1) for data
