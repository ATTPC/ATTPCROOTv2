...
void readSi()
{
   ...
   TH2F *h2_dede = new TH2F("h2_dede", "E-Loss in double layers silicon", 1000, 0, 1000, 1000, 0, 1000);

   for (int runNum: runNums) {
      // Open the digitalization file and get the TTree.
      TString unpackFileName = TString::Format("/data/ATTPCROOTv2_results/E565/UnpackerOutput/run_%04d_onlySiData.root", runNum);
      ...
      for (int i = 0; i < nUnpackEvents; i++) {
         ...
         // First, we obtain the hit information by AtRawEvent.
         AtRawEvent *rawEvent = (AtRawEvent*) rawArray->At(0);
         if (!rawEvent) continue;
	 
	 int nhits = rawEvent->GetNumAuxPads();
	 double eneSi;
	 for(int ihit = 0; ihit < nhits; ihit++) {
		 AtPad *rawPad = (AtPad*) rawEvent->GetPad(ihit);

		 idSi = rawPad->GetPadNum();
		 eneSi = rawPad->GetADC();
	 }
	
            h2_dede->Fill(eneSi1, eneSi2);
         }
