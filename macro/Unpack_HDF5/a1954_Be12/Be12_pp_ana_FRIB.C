Double_t GetNPeaksHRS(std::vector<Int_t> *timeMax, std::vector<Float_t> *adcMax, double *adc_test)
{
   ...
   Int_t nfound;
   nfound = s->SearchHighRes(adc_test, dest, 2048, 5, 5, kFALSE, 1, kFALSE, 1);
   ...
   for (auto iPeak = 0; iPeak < nfound; ++iPeak) {
      ...
      // std::cout<<" Time "<<time<<" - Max "<<adc_test[time]<<"\n";
   }
...
void Be12_pp_ana_FRIB(TString fileName = "run_0159_FRIB")
{
