void run_hierarchical_clustering(Int_t firstEvent = 0, Int_t eventCount = std::numeric_limits<Int_t>::max())
{
	...
	TString inputFile = "/home/dani/Escritorio/runrootHieratical/run_0280.root";
	TString outputFile = "/media/dani/ubuntuwindows/UBUNTU/e15250/Hieratical/data/output";

        std::ofstream outputFilecurvas("data/curvas.txt"); 
	...
	for (Int_t i = firstEvent; i < lastEvent; i += (isSimulation ? 2 : 1)) {
		...
		outputFilecurvas<< "# Analyzing event " << (i + 1) << " of " << eventsBranch->GetEntries() << std::endl;
		...
		try
		{
			...
			outputFilecurvas << "  expected vertex-position: " << vertex(0) << " " << vertex(1) << " " << vertex(2) << std::endl;
			...
			for (ATTrajectory const &trajectory : trajectories)
			{
				std::cout << "  ## TRAJECTORY ##" << std::endl;
       				outputFilecurvas << "  ## TRAJECTORY ##" << std::endl;
				...
				outputFilecurvas<< "      hit: " << hit.GetTrackID() << " (" << hit.GetPosition().X() << " " << hit.GetPosition().Y() << " " << hit.GetPosition().Z() << ")" <<"\n";
				}
				...
				std::cout << "    splineStartPosition: " << splineStartPosition << std::endl;
                                ...
                                outputFilecurvas<< "    splineStartPosition: " << splineStartPosition << std::endl;
