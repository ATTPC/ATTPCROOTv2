...
void C16_aa_ana()
{
   ...
   std::vector<TString> files{"run_0211.root"};
   files.push_back("run_0212.root");
      ...
      for (Int_t i = 0; i < nEvents; i++) {
         ...
         if (patternEvent) {
            ...
            for (auto index = 0; index < patternTrackCand.size(); ++index) {
               ...
               angle_vs_energy->Fill(theta * TMath::RadToDeg(), ener * Am);
               auto [ex_energy_exp, theta_cm] = kine_2b(m_C16, m_a, m_b, m_B, Ebeam_buff, theta, ener * Am);

               HQval->Fill(ex_energy_exp);

               // Excitation energy vs Beam energy
               for (auto iEb = 0; iEb < 300; ++iEb) {
                  auto [Qdep, theta_cm_qdep] = kine_2b(m_C16, m_a, m_b, m_B, iEb, theta, ener * Am);
                  QvsEb->Fill(Qdep, iEb);
               }
