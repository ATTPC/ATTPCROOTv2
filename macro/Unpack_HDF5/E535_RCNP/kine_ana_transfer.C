...
void kine_ana_transfer(int runNum)
{
   ...
   Int_t track_num;
   Double_t kineE[20];
   Double_t thetaLab[20];
   Double_t phiLab[20];
   Double_t range[20];
   Double_t range_small[20];
   Double_t dedx[20];
   Double_t dedx_small[20];
   ...
   TString output_filename = TString::Format("./transfer_data/simple_%d.root", runNum);
   ...
   tree_new->Branch("kineE", kineE, "kineE[track_num]/D");
   ...
   tree_new->Branch("range_small",range_small,"range_small[track_num]/D");
   tree_new->Branch("dedx", dedx, "dedx[track_num]/D");
   tree_new->Branch("dedx_small", dedx_small, "dedx_small[track_num]/D");
   tree_new->Branch("SiFe1", &SiFe1, "SiFe1/D");
   tree_new->Branch("SiFe2", &SiFe2, "SiFe2/D");

   tree_new->Branch("GAGGnum1", &GAGGnum1, "GAGGnum/I");
   ...
   for (int iEvent = 0; iEvent < nUnpackEvents; iEvent++) {
      ...
      for (auto &track: tracks){
         ...
         for (auto &hit: hits){
            ...
            if(currentRangeInSmallPads < 160 && currentRangeInSmallPads > 50){
               dEdx_small = smallPadCharge / currentRangeInSmallPads;
            }
         }
         ...
         kineE[track_num] = estimatedKinE;
         thetaLab[track_num] = trackThetaLAB;
         phiLab[track_num] = trackPhi;
         range[track_num] = roughRangeEstimation;
         dedx[track_num] = dEdx;
         dedx_small[track_num] = dEdx_small;
         range_small[track_num] = rangeInSmallPads;
         track_num += 1;

      }
      tree_new->Fill();
      if(iEvent % 1000 == 0)cout<<"processing : "<<iEvent<<" / "<<nUnpackEvents<<endl;
   }
