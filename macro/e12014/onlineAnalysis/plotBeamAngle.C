...
void plotBeamEnergyLost(int nscl_run_num = 310, int tpc_run_num = 118)
{
  ...
  tpc_tree.AddFriend("tr", TString::Format("/mnt/analysis/e12014/HiRAEVT/Calibrated/run-%02d-aligned.root", nscl_run_num));
  ...
  for(int i = 0; i < cuts.size(); ++i) hists.emplace_back(TString::Format("hist%d;Time bucket;ADC signal all pads", i), "", num_tb+1, 0, num_tb);
  ...
  while(reader.Next())
  {
    ...
    if(id == cuts.size()) continue; // if search reaches the end
    auto& hist = hists[id];
...
void plotBeamEnergyVsIC(int nscl_run_num = 310, int tpc_run_num = 118)
{
  ...
  while(reader.Next())
  {
    ...
    //if(num_valid_tracks == 2)
    {
      ...
      {
        auto adc = pad.GetRawADC();
        for(int i = 120; i < num_tb - 50; ++i) adc_sum += adc[i];
      }
