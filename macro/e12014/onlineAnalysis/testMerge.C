...
void testMerge(int nsclRunNum, int tpcRunNum)
{
  ...
  TString nsclFile  = TString::Format("/mnt/analysis/e12014/HiRAEVT/unpacked/run-%04d-00.root",
				      nsclRunNum);
  TString attpcFile = TString::Format("/mnt/analysis/e12014/TPC/unpacked/run_%04d.root",
				      tpcRunNum);
  TString mergeFile = TString::Format("/mnt/analysis/e12014/HiRAEVT/merged/run-%04d.dat",
				      nsclRunNum);
...
/*
  ...
  */
  ...
  for (auto i = 0; i < nEvents-1; ++i)
  {
    ...
    //std::cout << i << "," << tpcEvent->GetEventID() << "," << nsclTS[1] << "," << tpcEvent->GetTimestamp() << "," << y[i] << std::endl;
    if(i == nEvents/100)
