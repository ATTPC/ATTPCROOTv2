...
void PlotPID(AtTabInfo *info)
{
   ...
   TFile *cutFile = new TFile("/mnt/projects/hira/e12014/tpcSharedInfo/PIDcuts/Run323Yield.root");

   auto keys = cutFile->GetListOfKeys();
   for (auto key : *keys) {
      auto name = key->GetName();
      auto *cut = static_cast<TCutG *>(cutFile->GetObjectChecked(name, "TCutG"));
      cut->Draw("SAME");
      double x = 0;
      double y = 0;
      cut->Center(x, y);
