...
#ifndef ATPXCTDIGITIZER_H
#define ATPXCTDIGITIZER_H
...
class AtPxCTCrystalCalData;
class TBuffer;
class TClass;
class TMemberInspector;

class AtPxCTDigitizer : public FairTask {
private:
   TClonesArray *fMCPointDataCA;   //!  The crystal hit collection (we do not own)
   TClonesArray fPxCTCryCalDataCA; /**< Array with CALIFA Cal- output data. >*/
...
public:
    ...
    **
    ** Adds a PxCTCrystalCal data
    **/
   AtPxCTCrystalCalData *AddCrystalCal(Int_t ident, Double_t energy, ULong64_t time);
