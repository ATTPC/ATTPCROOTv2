...
void AtFindVertex::FindVertexSingleLine(std::vector<AtTrack> tracks)
{
  ...
  for (auto track : tracks) {
    ...
    if (patternPar.size()==0 || nanPattern) continue;
...
void AtFindVertex::FindVertexMultipleLines(std::vector<AtTrack> tracks, Int_t nbTracksPerVtx)
{
  ...
  for (auto track : tracks) {
    ...
    for (Int_t i = 0; i < vtxCand.size(); i++) {
      ...
      if(vtxCand.at(i).size()>nbTracksPerVtx) std::cout<<cYELLOW<<" vtx with more than "<<nbTracksPerVtx<<" tracks("<<vtxCand.at(i).size()<<")"<<cNORMAL<<std::endl;
      std::vector<AtTrack> tracksVtx;
...
std::vector<std::vector<Int_t>> AtFindVertex::SortTrackSameVtx(std::vector<std::vector<Double_t>> lines)
{
      ...
      std::vector<Double_t> line = lines.at(i);
                ...
                for(Int_t j=i+1; j<lines.size();j++)
                {
                  ...
                  if(vtx.size()==0) {//if no vertex found
                    ...
                    // std::cout<<" paired size "<<paired.size()<<" "<<i<<" "<<j<<" "<<std::endl;
                    XYZVector projVtx;
                    projVtx = ptOnLine(line_f, vtx.back());//vtx.back() assumes one only wants events with one vertex, but it can be modified
                    Double_t dist1 = sqrt((vtx.back()-projVtx).Mag2());//distance between saved vtx and its projection on the new line
                    Double_t radProjVtx = sqrt((0.5*(vtx.back()+projVtx)).Perp2());
                    //std::cout<<radProjVtx<<" distlines "<<dist1<<" vtxBack "<<vtx.back().X()<<" "<<vtx.back().Y()<<" "<<vtx.back().Z()<<" "<<projVtx.X()<<" "<<projVtx.Y()<<" "<<projVtx.Z()<<std::endl;
                    if(dist1 < fLineDistThreshold && radProjVtx<=30.){//more than 2 lines from a same vertex in the beam region
                        if(std::find(paired.begin(), paired.end(), i)==paired.end()){
                          paired.push_back(i);
                        }
                        paired.push_back(j);
  ...
  if(paired.size()>1) result.push_back(paired);
...
std::vector<std::pair<Int_t, XYZVector>> AtFindVertex::CoGVtxSingleTrack(std::vector<std::vector<Double_t>> lines, std::vector<Int_t> itracks)
{
  ...
  for(Int_t i=0;i<lines.size();i++)
  {
    ...
    //std::cout<<lines.size()<<" check lines size "<<std::endl;
      ...
      if(dist < fLineDistThreshold){
          ...
          vtx = projVtxOnLines1.at(0);//this way the track angle is better than taking the CoG, the uncertainty on the range is not improved
                    ...
                    std::vector<Double_t> line_f = lines.at(jj);
                          ...
                          if(dist < fLineDistThreshold){
                            if(angle<10 || angle>170){
                              ...
                              //std::cout<<"check CoG "<<line.at(0)<<" "<<line.at(1)<<" "<<line_f.at(0)<<" "<<line_f.at(1)<<std::endl;
                            }
                          }
...
Bool_t AtFindVertex::checkExclusivity(std::vector<Int_t> v1, std::vector<Int_t> v2)
{
    ...
    if(v1.size()!=diff.size()) result=kFALSE;
