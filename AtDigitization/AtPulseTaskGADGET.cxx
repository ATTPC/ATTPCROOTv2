...
Double_t AtPulseTaskGADGET::ChargeDispersion(Double_t G,Double_t time, Double_t x0,Double_t y0,Double_t xi,Double_t yi) { 
   ...
   Double_t Charge = G/4*(TMath::Erf((xi+W/2-x0)/(rtTwo*Sigma))-TMath::Erf((xi-W/2-x0)/(rtTwo*Sigma)))*
...
for (Int_t i = 0; i < Items; i++) {
  ...
  for (Int_t j = 0; j < Items; j++) {
    ...
    auto newbinNumber = fPadPlane->Fill(xPadCurrent, yPadCurrent);
    auto newpadNumber = fMap->BinToPad(newbinNumber);

    if (newpadNumber < 0 || newpadNumber >= numPads) {
