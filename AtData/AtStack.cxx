...
AtStack::AtStack(Int_t size)
  : FairGenericStack(),
    ...
    fTracks(new TClonesArray("AtMCTrack", size)),
    fStoreMap(),
    fStoreIter(),
    fIndexMap(),
    fIndexIter(),
...
void AtStack::PushTrack(Int_t toBeDone, Int_t parentId, Int_t pdgCode,
...
void AtStack::FillTrackArray()
{
  ...
  for (Int_t iPart=0; iPart<fNParticles; iPart++) {
    ...
    if (store) {
      AtMCTrack* track =
        new( (*fTracks)[fNTracks]) AtMCTrack(GetParticle(iPart));
...
{
  cout << "-I- AtStack: Number of primaries        = "
       << fNPrimaries << endl;
