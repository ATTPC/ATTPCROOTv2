...
/*********************************************************************
 *   SpecMAT Mapping Class	AtSpecMATMap.cxx		     *
 *   Author: Oleksii Poleshchuk 				     *
 *   Addapted by  B. Olaizola            			     *
 *   Log: 03-03-2021 17:16 CET					     *
 *								     *
 *********************************************************************/

#include "AtSpecMATMap.h"

static int nTriangle=3168;//number of triangle in the padplane 
...
{
  AtPadCoord.resize(boost::extents[nTriangle][3][2]);
  kIsParsed=0;
  kGUIMode=0;
  kDebug=0;
...
std::vector<double> TrianglesGenerator() {
  ...
  double x0[arrayLength], x1[arrayLength], x2[arrayLength], xCentre[arrayLength], x0next[arrayLength], x1next[arrayLength], x2next[arrayLength] ;
  double y0[arrayLength], y1[arrayLength], y2[arrayLength], yCentre[arrayLength], y0next[arrayLength], y1next[arrayLength], y2next[arrayLength] ;
  double h = 4.725;
  ...
  for (int i = 1; i < nSegments; i++) {
    for (int j = 0; j < nElementsInSegment; j++) {
      x0[i*nElementsInSegment+j] = 0.5*(x0[(i-1)*nElementsInSegment+j]-0)-sqrt(3)*0.5*(y0[(i-1)*nElementsInSegment+j]);
      y0[i*nElementsInSegment+j] = sqrt(3)*0.5*(x0[(i-1)*nElementsInSegment+j]-0)+0.5*(y0[(i-1)*nElementsInSegment+j]);

      x1[i*nElementsInSegment+j] = 0.5*(x1[(i-1)*nElementsInSegment+j]-0)-sqrt(3)*0.5*(y1[(i-1)*nElementsInSegment+j]);
      y1[i*nElementsInSegment+j] = sqrt(3)*0.5*(x1[(i-1)*nElementsInSegment+j]-0)+0.5*(y1[(i-1)*nElementsInSegment+j]);

      x2[i*nElementsInSegment+j] = 0.5*(x2[(i-1)*nElementsInSegment+j]-0)-sqrt(3)*0.5*(y2[(i-1)*nElementsInSegment+j]);
      y2[i*nElementsInSegment+j] = sqrt(3)*0.5*(x2[(i-1)*nElementsInSegment+j]-0)+0.5*(y2[(i-1)*nElementsInSegment+j]);

      xCentre[i*nElementsInSegment+j] = 0.5*(xCentre[(i-1)*nElementsInSegment+j]-0)-sqrt(3)*0.5*(yCentre[(i-1)*nElementsInSegment+j]);
      yCentre[i*nElementsInSegment+j] = sqrt(3)*0.5*(xCentre[(i-1)*nElementsInSegment+j]-0)+0.5*(yCentre[(i-1)*nElementsInSegment+j]);
    }
  }
...
TH2Poly *SpecMATHistoGenerator(){
  ...
  for (int i2 = 0; i2 < arrayAllCoordinatesSize/8; i2++) {
    if (i2 != 0 && i2 != arrayAllCoordinatesSize*1/(6*8) && i2 != arrayAllCoordinatesSize*2/(6*8) && i2 != arrayAllCoordinatesSize*3/(6*8) && i2 != arrayAllCoordinatesSize*4/(6*8) && i2 != arrayAllCoordinatesSize*5/(6*8)) {
...
void AtSpecMATMap::GenerateSpecMAT()
{   
...
TH2Poly* AtSpecMATMap::GetATTPCPlane(){
  ...
  cout << "Input" << endl;
  cout << "X: " << InitialPoInt_X << " Y: " << InitialPoInt_Y << endl;
  cout << "\nOutput" << endl;
  cout << "Bin: " << currentBin << " binCentroidX: " << binXcentroid << " binCentroidY: " << binYcentroid << endl;

  SpecMAThisto->SetName("SpecMAT_Plane");
  SpecMAThisto->SetTitle("SpecMAT_Plane");
  SpecMAThisto->ChangePartition(500,500);

  if(kGUIMode){
    cSpecMATPlane = new TCanvas("cSpecMATPlane","cSpecMATPlane",1000,1000);
    gStyle->SetPalette(1);
    SpecMAThisto->Draw("COL L");
  }

  return SpecMAThisto;

}//AtSpecMATMap::GetATTPCPlane()
