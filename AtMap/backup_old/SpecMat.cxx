...
void AtSpecMATMap::Dump(){
  ...
  for(index i = 0; i != 10240; ++i){
    ...
    for(index j = 0; j != 3; ++j){
      for(index k = 0; k != 2; ++k){
	std::cout<<" SpecMAT Triangular pad coordinates - Pad Index : "<<i<<"   X("<<j<<")  -  Y("<<k<<") :"<<AtPadCoord[i][j][k]<<std::endl;
...
void AtSpecMATMap::GenerateSpecMAT(){
  ...
  std::cout<<" SpecMAT Map : Generating the map geometry of the SpecMAT "<<std::endl;
  // Local variables
...
Int_t AtSpecMATMap::fill_coord(int pindex, float padxoff, float padyoff, float triside, float fort){
...
TH2Poly* AtSpecMATMap::GetSpecMATPlane(){

  if(fPadInd == 0){

    std::cout<<" AtSpecMATMap::GetSpecMATPlane Error : Pad plane has not been generated - Exiting... "<<std::endl;
  ...
  hPlane->SetName("SpecMAT_Plane");
  hPlane->SetTitle("SpecMAT_Plane");
...
std::vector<Float_t> AtSpecMATMap::CalcPadCenter(Int_t PadRef){
  ...
  if(PadRef!=-1){ //Boost multi_array crashes with a negative index
    ...
    if(kDebug) std::cout<<" AtSpecMATMap::CalcPadCenter Error : Pad not found"<<std::endl;
    return PadCenter;
