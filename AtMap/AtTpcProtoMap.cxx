...
std::vector<Float_t> AtTpcProtoMap::CalcPadCenter(Int_t PadRef){
		...
		Int_t kIs = Int_t(ProtoGeoMap.find(PadRef) == ProtoGeoMap.end());
		  if(kIs){
                    if(kDebug)std::cerr<<" AtTpcProtoMap::CalcPadCenter - Pad  not found - CoboID : "<<PadRef<<std::endl;
                    return PadCenter;
                   }
		...
		if(kDebug) std::cout<<" AtTpcProtoMap::CalcPadCenter Error : Pad not found"<<std::endl;
		return PadCenter;
