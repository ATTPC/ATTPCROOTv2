...
Int_t AtMap::GetPadNum(std::vector<int> PadRef){ //TODO Better to pass a pointer!
...
//std::cout<<int(ATTPCPadMap.find(test) == ATTPCPadMap.end())<<endl;
  ...
  {
    if(kDebug)
      std::cerr<<" AtTpcMap::GetPadNum - Pad key not found - CoboID : "<<PadRef[0]
	       <<"  AsadID : "<<PadRef[1]<<"  AgetID : "<<PadRef[2]<<"  ChannelID : "<<PadRef[3]<<std::endl;
    return -1;
  }
