...
{
   return (UInt_t)fMetaType;
}
UInt_t GETHeaderBase::GetFrameSize(Bool_t inBytes)
{
   return CorrectEndianness(fFrameSize, 3) * (inBytes ? GetUnitBlock() : 1);
}
UInt_t GETHeaderBase::GetDataSource()
{
   return (UInt_t)fDataSource;
}
UInt_t GETHeaderBase::GetFrameType()
{
   return CorrectEndianness(fFrameType, 2);
}
UInt_t GETHeaderBase::GetRevision()
{
   return (UInt_t)fRevision;
}
ULong64_t GETHeaderBase::GetFrameSkip(Bool_t rewind)
{
   return GetFrameSize() - GETHEADERBASESIZE * (!rewind);
}

Bool_t GETHeaderBase::IsLittleEndian()
{
   return ((GetMetaType() & 0x80) >> 7);
}
Bool_t GETHeaderBase::IsBlob()
{
   return ((GetMetaType() & 0x40) >> 6);
}
UInt_t GETHeaderBase::GetUnitBlock()
{
   return pow(2, GetMetaType() & 0xf);
}
...
void GETHeaderBase::Print()
{
   ...
   cout << "   frameSize: " << GetFrameSize(false) << " (" << dec << GetFrameSize(false)
