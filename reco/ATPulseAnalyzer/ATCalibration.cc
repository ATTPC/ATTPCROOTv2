...
void ATCalibration::SetGainFile(TString gainFile)
{
    ...
    if (gainData->fail()) {
	std::cout <<
	    " =  No Gain Calibration file found! Please, check the path. Current :"
	    << fGainFile.Data() << std::endl;
	std::cout << cRED << " =  Proceeding with no gain calibration!!" <<
	    cNORMAL << std::endl;
	fIsGainCalibrated = kFALSE;
    } else {
	std::cout << " == Gain calibration using: " << cRED << fGainFile.Data()
	    << cNORMAL << std::endl;
...
void ATCalibration::SetJitterFile(TString jitterFile)
{
    ...
    if (jitterData->fail()) {
	std::cout <<
	    " = No Jitter Calibration file found! Please check the path. Current :"
	    << cNORMAL << fJitterFile.Data() << std::endl;
	std::cout << cRED << " = Proceeding with no jitter calibration!!" <<
	    cNORMAL << std::endl;
	fIsJitterCalibrated = kFALSE;
    } else {
	std::cout << " == Jitter calibration using: " << cRED <<
	    fJitterFile.Data() << cNORMAL << std::endl;
