 ...
 ATPSAProtoFull::Analyze(ATRawEvent * rawEvent, ATEvent * event)
{
	...
	for (Int_t iTb = 0; iTb < fNumTbs; iTb++) {
		...
		if (iTb > fIniTB && iTb < fEndTB)
		    QEventTot += floatADC[iTb];	//This allows to constrain the calculation of the charge avoiding noisy timebuckets
		...
		if ((xPos < -9000 || yPos < -9000) && pad->GetPadNum() != -1)
		    std::cout <<
			" ATPSAProtoFull::Analysis Warning! Wrong Coordinates for Pad : "
