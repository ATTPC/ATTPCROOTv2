...
vector<double> ATMlesacMod::GetPDF(const std::vector<int>  samplesIdx){
...
void ATMlesacMod::EstimModel(const std::vector<int>  samplesIdx)
{
...
double ATMlesacMod::EstimError(int i)
{
...
void ATMlesacMod::CalcMlesacMod(ATEvent *event)
{

    //std::cout << "Inicializa" << '\n';
    Init(event);
    //std::cout << "Resuelve" << '\n';
    if(vX.size()>fMlesacMinPoints){
      Solve();
      ...
      //std::cout<<"MlesacMod tracks size : "<<tracksSize<<std::endl;
...
void ATMlesacMod::SetCluster(const std::vector<int> samplesIdx, const double cost, const double Chi2, TVector3 CP1, TVector3 CP2)
{
    Cluster cstr;
...
std::vector<ATTrack*> ATMlesacMod::Clusters2Tracks( AllClusters NClusters, ATEvent *event)
{
...
void ATMlesacMod::FindVertex(std::vector<ATTrack*> tracks)
{
...
void ATMlesacMod::FindVertexOneTrack(std::vector<ATTrack*> tracks)
{
