...
vector<double> ATLmedsMod::GetPDF(const std::vector<int>  samplesIdx){
...
vector<int> ATLmedsMod::RandSam(vector<int> indX, Int_t mode)
{
...
void ATLmedsMod::EstimModel(const std::vector<int>  samplesIdx)
{
...
double ATLmedsMod::EstimError(int i)
{
...
void ATLmedsMod::CalcLmedsMod(ATEvent *event)
{

    //std::cout << "Inicializa" << '\n';
    Init(event);
    //std::cout << "Resuelve" << '\n';
    if(vX.size()>fLmedsMinPoints){
      Solve();
      ...
      //std::cout<<"LmedsMod tracks size : "<<tracksSize<<std::endl;
...
void ATLmedsMod::SetCluster(const std::vector<int> samplesIdx, const double cost, const double Chi2, TVector3 CP1, TVector3 CP2)
{
    Cluster cstr;
...
std::vector<ATTrack*> ATLmedsMod::Clusters2Tracks( AllClusters NClusters, ATEvent *event)
{
  ...
  int numclus = NClusters.size();
	...
	for(int i = 0; i < numclus; i++){
      ...
      //std::cout << "hits en el track   "<<track->GetHitArray()->size()<<"   "<< track->GetHitArray()->at(0).GetTrackID()<< '\n';
...
void ATLmedsMod::FindVertex(std::vector<ATTrack*> tracks)
{
...
Double_t ATLmedsMod::Fit3D(vector<int> inliners, TVector3& V1, TVector3& V2)
{
