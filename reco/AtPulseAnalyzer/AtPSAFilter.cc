...
void AtPSAFilter::Analyze(AtRawEvent * rawEvent, AtEvent * event)
{
    ...
    pcl::PointCloud < pcl::PointXYZRGBA >::Ptr cloud(new pcl::PointCloud <
						     pcl::PointXYZRGBA >);
    pcl::PointCloud <
	pcl::PointXYZRGBA >::Ptr cloud_filtered(new pcl::PointCloud <
						pcl::PointXYZRGBA >);
	...
	if (fIsPeakFinder)
	    numPeaks =
		PeakFinder->SearchHighRes(floatADC, dummy, fNumTbs, 4.7, 5,
					  fBackGroundSuppression, 3, kTRUE, 3);
	if (fIsMaxFinder)
	    numPeaks = 1;

	TSpectrum *BGInter = new TSpectrum;
	if (fBackGroundInterp) {
	    BGInter->Background(bg, fNumTbs, 6,
	...
	if (fValidBuff) {
		...
		if (fIsPeakFinder) {
		    maxAdcIdx =
			(Int_t) (ceil((PeakFinder->GetPositionX())[iPeak]));
		...
		Double_t timemax =
		    0.5 * (floatADC[maxAdcIdx - 1] -
			   floatADC[maxAdcIdx + 1]) / (floatADC[maxAdcIdx - 1] +
						       floatADC[maxAdcIdx + 1] -
						       2 * floatADC[maxAdcIdx]);
		...
		if (maxAdcIdx > 11) {
			...
			TB_TotQ +=
			    floatADC[maxAdcIdx - i + 5] - basecorr / 10.0;
		...
		if (fValidThreshold && fValidDerivative) {
		    ...
		    if (iPeak == 0)
			QEventTot += QHitTot;	//Sum only if Hit is valid - We only sum once (iPeak==0) to account for the whole spectrum.

		    TVector3 posRot =
			RotateDetector(xPos, yPos, zPos, maxAdcIdx);

		    AtHit *hit =
			new AtHit(PadNum, hitNum, xPos, yPos, zPos, charge);
		    ...
		    Rho2 += HitPos.Mag2();
		    RhoMean += HitPos.Mag();
		    if ((xPos < -9000 || yPos < -9000)
			&& pad->GetPadNum() != -1)
			std::cout <<
			    " AtPSASimple2::Analysis Warning! Wrong Coordinates for Pad : "
	    ...
	    //#pragma omp ordered
	    PadMultiplicity.insert(std::pair < Int_t,
				   Int_t > (PadNum, PadHitNum));
    ...
    RhoVariance =
	Rho2 - (event->GetNumHits() * pow((RhoMean / event->GetNumHits()), 2));
