...
void AtPSAProto::Analyze(AtRawEvent * rawEvent, AtEvent * event)
{
	...
	if (fValidBuff) {
		...
		if (maxAdcIdx > 11) {
			...
			TBCorr +=
			    floatADC[maxAdcIdx - i + 5] * (maxAdcIdx - i + 5);
		...
		if (fValidThreshold) {
			...
			for (Int_t iTb = fIniTB; iTb < fEndTB; iTb++)
				...
				QEventTot += adc[iTb];	//Sum only if Hit is valid - We only sum once (iPeak==0) to account for the whole spectrum.
			...
			&& pad->GetPadNum() != -1)
			std::cout <<
			    " AtPSAProto::Analysis Warning! Wrong Coordinates for Pad : "
