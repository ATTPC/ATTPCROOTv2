...
// ------   Constructor   --------------------------------------------------
AtFieldPar::AtFieldPar(const char *name, const char *title, const char *context)
   : FairParGenericSet(name, title, context), fType(-1), fXmin(0.), fXmax(0.), fYmin(0.), fYmax(0.), fZmin(0.),
     fZmax(0.), fBx(0.), fBy(0.), fBz(0.), fMapName(""), fPosX(0.), fPosY(0.), fPosZ(0.), fScale(0.), fPeak(0.),
     fMiddle(0.)
...
AtFieldPar::AtFieldPar()
   : FairParGenericSet(), fType(-1), fXmin(0.), fXmax(0.), fYmin(0.), fYmax(0.), fZmin(0.), fZmax(0.), fBx(0.), fBy(0.),
     fBz(0.), fMapName(""), fPosX(0.), fPosY(0.), fPosZ(0.), fScale(0.), fPeak(0.), fMiddle(0.)
...
void AtFieldPar::SetParameters(FairField *field)
{
   ...
   if (!field) {
      cerr << "-W- AtFieldPar::SetParameters: Empty field pointer!" << endl;
      return;
   }
   ...
   if (fType == 0) { // constant field
      ...
      cerr << "-W- AtFieldPar::SetParameters: Unknown field type " << fType << "!" << endl;
      fBx = fBy = fBz = 0.;
