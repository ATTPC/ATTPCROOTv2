...
{
   if (fUseRandom1)
      fRandomEngine = new PyTr1Rng();
   if (fUseRandom3)
      fRandomEngine = new PyTr3Rng();
...
Bool_t Pythia8Generator::ReadEvent(FairPrimaryGenerator *cpg)
{
   ...
   // cout<<"debug p8 event 0 " << fPythia.event[0].id()<< " "<< fPythia.event[1].id()<< " "<< fPythia.event[2].id()<< "
   // "<< npart <<endl;
   for (Int_t ii = 0; ii < fPythia.event.size(); ii++) {
      if (fPythia.event[ii].isFinal()) {
         ...
         if (wanttracking) {
            ...
            cpg->AddTrack((Int_t)fPythia.event[ii].id(), px, py, pz, x, y, z, (Int_t)fPythia.event[ii].mother1(),
                          wanttracking);
            // cout<<"debug p8->geant4 "<< wanttracking << " "<< ii <<  " " << fPythia.event[ii].id()<< " "<<
            // fPythia.event[ii].mother1()<<" "<<x<<" "<< y<<" "<< z <<endl;
         }
         //    virtual void AddTrack(Int_t pdgid, Double_t px, Double_t py, Double_t pz,
         //                          Double_t vx, Double_t vy, Double_t vz, Int_t parent=-1,Bool_t
         //                          wanttracking=true,Double_t e=-9e9);
      };
