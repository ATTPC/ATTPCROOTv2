...
# Print out summary of possible configuration types
  if(CMAKE_CONFIGURATION_TYPES)
	...
	pad("${type}" 18 " " type_padded)
	message(STATUS "  ${BWhite}${type_padded}${ColourReset}${CMAKE_CXX_FLAGS_${type_upper}}")
      endif()
      unset(type_padded)
      unset(type_upper)
    endforeach()
...
function(PrintDependencies)
  ...
  if(PROJECT_PACKAGE_DEPENDENCIES)
    message(STATUS "  ")
    message(STATUS "  ${Cyan}DEPENDENCY FOUND     VERSION                   PREFIX${ColourReset}")
    foreach(dep IN LISTS PROJECT_PACKAGE_DEPENDENCIES)

      if(${dep}_VERSION)
	if(${dep} STREQUAL Boost)
          if(Boost_VERSIONg_MAJOR)
            set(version_str "${BGreen}${${dep}_VERSION_MAJOR}.${${dep}_VERSION_MINOR}${ColourReset}")
          else()
            set(version_str "${BGreen}${${dep}_MAJOR_VERSION}.${${dep}_MINOR_VERSION}${ColourReset}")
          endif()
	else()
          set(version_str "${BGreen}${${dep}_VERSION}${ColourReset}")
	endif()
      else()
	set(version_str "${BYellow}unknown${ColourReset}")
      endif()
	...
	get_filename_component(prefix "${FAIRROOT_LIBRARY_DIR}/.." ABSOLUTE)
      ...
      message(STATUS "  ${BWhite}${dep_padded}${ColourReset}${version_padded}${prefix}")
  ...
  message(STATUS "  ")
  message(STATUS "  ${Red}OPTIONAL DEPENDENCY NOT FOUND${ColourReset}")
  foreach(dep IN LISTS PROJECT_PACKAGE_NOT_FOUND)
...
function(PrintStaticAnalyzers)
  ...
  if(RUN_STATIC_ANALYSIS)
    ...
    message(STATUS "  (Disable individual with  ${BMagenta}-DPROJECT_analyzer_DISABLE=ON${ColourReset})")
    message(STATUS "  ")
    message(STATUS "  ${Cyan}ANALYZER     ENABLED      FOUND      EXE${ColourReset}")
    
    foreach(analyzer IN LISTS PROJECT_STATIC_ANALYZERS)
      pad(${analyzer} 12 " " name_padded)

      if(${PROJECT_${analyzer}_DISABLE})
	pad("${BRed}NOT ENABLED${ColourReset}" 12 " " enabled_padded COLOR 1)
      else()
	pad("${BGreen}ENABLED${ColourReset}" 12 " " enabled_padded COLOR 1)
      endif()
      
      if(${analyzer}_FOUND)
	pad("${BGreen}FOUND${ColourReset}" 10 " " found_padded COLOR 1)
      else()
	pad("${BRed}NOT FOUND${ColourReset}" 10 " " found_padded COLOR 1)
      endif()

      set(${analyzer}_status "${name_padded}${enabled_padded}${found_padded}${${analyzer}_path}")
      message(STATUS "  ${${analyzer}_status}")
    endforeach()

  else(RUN_STATIC_ANALYSIS)
