...
Bool_t ATMergeTask::isInGlom(Double_t ts1, Double_t ts2)
{
  bool is=false;
...
*/
  ...
  */
  ...
  TGraph *gS800 = new TGraph(fTsEvtS800Size, &fS800Ts[0], &fS800Evt[0]);
...
//  fS800Br -> Delete();
  ...
  Double_t ATTPCTs = rawEvent -> GetTimestamp();
  ...
  for(int i=0;i<fTsEvtS800Size;i++)
  {
    if(i>0 && isInGlom(fS800Ts.at(i-1),fS800Ts.at(i)) )std::cout<<" -- Warning -- Timestamp of consecutive entries from S800 root file within the glom"<<std::endl;
      ...
      minj=fOptiFit->Eval(ATTPCTs)-fEvtDelta;//define the ATTPC entries range where the matching timestamp should be, to not loop over all the ATTPC entries.
      maxj=fOptiFit->Eval(ATTPCTs)+fEvtDelta;
      ...
      else*/ if(j>0 && isInGlom(fS800Ts.at(i),ATTPCTs) )
      {
        S800EvtMatch = j;
        //if(i<2) std::cout<<" in glom "<<minj<<" "<<maxj<<" "<<i<<" "<<j<<" "<<fS800Ts.at(i)<<" "<<ATTPCTs<<std::endl;
        continue;
      }
      else
      S800EvtMatch = -1;
      //if(i<2)std::cout<<" not in glom "<<minj<<" "<<maxj<<" "<<i<<" "<<j<<" "<<fS800Ts.at(i)<<" "<<ATTPCTs<<std::endl;
    }
  }
}
