...
void AtHoughSpaceLine3D::lineTransform3D(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                         std::vector<std::pair<vector3D, vector3D>> &lines, unsigned int threshold,
                                         Sphere *sphere, double step_plane, unsigned int max_n)
{
   ...
   std::cout << "voting space size: " << num_plane * num_plane * num_b * sizeof(unsigned int) / 1024 / 1024 << " MB"
             << std::endl;
   ...
   if (max_n > 0 && votes.size() > max_n) {
      ...
      std::vector<std::pair<unsigned int, size_t>>(votes.begin() + votes.size() - max_n, votes.end())
         .swap(votes); // backwards resize()
   }
   ...
   for (size_t i = 0; i < votes.size(); i++) {
      ...
      std::cout << "  Votes : " << votes[i].first << " Anchor point : " << p.x << " " << p.y << " " << p.z
                << " Orientation vector :  " << b.x << " " << b.y << " " << b.z << std::endl;
      lines.push_back(std::pair<vector3D, vector3D>(p, b));
   }
}

void AtHoughSpaceLine3D::lineTransform3D_weighted(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                                  std::vector<double> &weights,
                                                  std::vector<std::pair<vector3D, vector3D>> &lines,
                                                  unsigned int threshold, Sphere *sphere, double step_plane,
                                                  unsigned int max_n)
{
   std::vector<double> sin_theta, cos_theta, sin_phi, cos_phi;
   unsigned int num_b, num_plane;
   ...
   std::cout << "Number of directional vectors: " << num_b << std::endl;
   std::cout << "Plane coordinate range: " << num_plane << std::endl;
   std::cout << "voting space size: " << num_plane * num_plane * num_b * sizeof(double) / 1024 / 1024 << " MB"
             << std::endl;
   ...
   if (max_n > 1) {
      AtHoughSpaceLine3D::nonmaximumSupression(VotingSpace, sphere->vertices, sphere->triangles, num_b, num_plane, 8,
                                               4);
   }
   ...
   if (max_n > 0 && votes.size() > max_n) {
      ...
      std::nth_element(votes.begin(), votes.begin() + votes.size() - max_n, votes.end()); // backward nth_element
      // cut off lines with less votes than nth line
      std::vector<std::pair<double, size_t>>(votes.begin() + votes.size() - max_n, votes.end())
         .swap(votes); // backwards resize()
   }
   // sort remaining lines in descending order
   std::sort(votes.rbegin(), votes.rend());
...
void AtHoughSpaceLine3D::lineTransform3D_fibonacciSphere(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                                         std::vector<std::pair<vector3D, vector3D>> &lines,
                                                         unsigned int threshold, unsigned int n_directionalVectors,
                                                         double step_plane, unsigned int max_n)
{
...
void AtHoughSpaceLine3D::lineTransform3D_azimuthElevation(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                                          std::vector<std::pair<vector3D, vector3D>> &lines,
                                                          unsigned int threshold, double step_b, double step_plane,
                                                          unsigned int max_n)
{
   //  Compute directional vectors
   Sphere *sphere = new Sphere();
...
void AtHoughSpaceLine3D::lineTransform3D_Tesselation(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                                     std::vector<std::pair<vector3D, vector3D>> &lines,
                                                     unsigned int threshold, unsigned char platonic_solid,
                                                     unsigned char granularity_b, double step_plane, unsigned int max_n)
{
   //  Compute directional vectors
   Sphere *sphere = new Sphere();
...
void AtHoughSpaceLine3D::lineTransform3D_Tesselation_weighted(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                                              std::vector<double> &weights,
                                                              std::vector<std::pair<vector3D, vector3D>> &lines,
                                                              unsigned int threshold, unsigned char platonic_solid,
                                                              unsigned char granularity_b, double step_plane,
                                                              unsigned int max_n)
{
   //  Compute directional vectors
   Sphere *sphere = new Sphere();
...
void AtHoughSpaceLine3D::nonmaximumSupression_AE(std::vector<unsigned int> &VotingSpace, Sphere *sphere,
                                                 unsigned int num_b, unsigned int num_plane, int n, int k)
{
         ...
         if (x_i + dx >= 0 && x_i + dx < num_plane) {
            for (int dy = -n; dy <= n; dy++) {
               if (y_i + dy >= 0 && y_i + dy < num_plane) {
                  // cout << "original : (" << x_i << "/" << y_i << ")   neu: (" << x_i + dx  << "/" << y_i + dy << ")"
                  // << endl;
                  std::vector<int>::iterator it = neighbourG[b_i].begin();
                  for (; it != neighbourG[b_i].end(); it++) {
                     if (*it == b_i && dx == 0 && dy == 0) {
                     ...
                     size_t index_n =
                        ((x_i + dx) * num_plane * num_b) + ((y_i + dy) * num_b) + *it; // retrieve neighbour's index
...
void AtHoughSpaceLine3D::nonmaximumSupression(std::vector<unsigned int> &VotingSpace, std::vector<vector3D> &vertices,
                                              std::deque<unsigned int> &triangles, unsigned int num_b,
                                              unsigned int num_plane, int n, int r)
{
   std::vector<unsigned int> VotingSpaceNew = VotingSpace;
...
void AtHoughSpaceLine3D::nonmaximumSupression(std::vector<double> &VotingSpace, std::vector<vector3D> &vertices,
                                              std::deque<unsigned int> &triangles, unsigned int num_b,
                                              unsigned int num_plane, int n, int r)
{
