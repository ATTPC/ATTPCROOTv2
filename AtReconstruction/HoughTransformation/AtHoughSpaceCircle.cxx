...
void AtHoughSpaceCircle::SetELossFuncArray(
   std::vector<std::function<Double_t(Double_t, std::vector<Double_t> &)>> &func_array)
{
   fEloss_func_array = func_array;
}

void AtHoughSpaceCircle::SetElossParameters(const std::vector<Double_t> &par)
{
   fElossPar = par;
}

std::pair<Double_t, Double_t> AtHoughSpaceCircle::GetHoughPar()
{
   return fHoughLinePar;
}

std::vector<Double_t> AtHoughSpaceCircle::GetRansacPar()
{
   return fRansacLinePar;
}

TH2F *AtHoughSpaceCircle::GetHoughSpace(TString ProjPlane)
{
   return HistHoughXY;
}
...
void AtHoughSpaceCircle::FillHistHoughXY(AtEvent *event, Int_t nstep)
{
   ...
   for (Int_t iHit = 0; iHit < (nHits - nstep); ++iHit) {
      ...
      for (Int_t itheta = 0; itheta < 1023; ++itheta) {
         ...
         Float_t const d0_XY_inv = 2.0f *
                                   (deltaPosForw.Y() * TMath::Sin(angle) + deltaPosForw.X() * TMath::Cos(angle)) /
                                   ((TMath::Power(position.Y(), 2) - TMath::Power(position_forw.Y(), 2)) +
                                    (TMath::Power(position.X(), 2) - TMath::Power(position_forw.X(), 2)));

         HistHoughXY->Fill(angle,
                           1.0f / d0_XY_inv); // TODO: flip calculation so we do not need to do the inversion here
      }
   }
}
...
void AtHoughSpaceCircle::Clustering(AtEvent *event)
{
   ...
   for (Int_t iHit = 0; iHit < (nHits - 1); ++iHit) {
      AtHit const &hit = event->GetHitArray()->at(iHit);
      TVector3 const position = hit.GetPosition();
      Double_t const geo_dist = TMath::Abs(TMath::Cos(fHoughLinePar.first) * fPhi->at(iHit) * fRadius->at(iHit) +
                                           TMath::Sin(fHoughLinePar.first) * hit.GetTimeStamp() - fHoughLinePar.second);
      Double_t const hit_dist =
         TMath::Sqrt(TMath::Power((fPhi->at(iHit) * fRadius->at(iHit) - IniPhiBuff * IniRadiusBuff), 2) +
      ...
      if (geo_dist < 10.0) {
         ...
         if (hit_dist < 10.0) {
            ...
            fIniHit->SetHit(hit.GetHitPadNum(), hit.GetHitID(), position.X(), position.Y(), position.Z(),
                            hit.GetCharge());
            // IniPosZ = position.Z();
...
void AtHoughSpaceCircle::CalculateParameters(AtEvent *event)
{
   ...
   for (Int_t iHit = 0; iHit < (nHits - 1); ++iHit) {
      ...
      // TVector2 const deltaPositionForw = TVector2(position_forw.X(), position_forw.Y()) - TVector2(position.X(),
      // position.Y());
      ...
      for (Int_t itheta = 0; itheta < 1023; ++itheta) {
         Float_t const angle = TMath::Pi() * (static_cast<Float_t>(itheta) / 1023.0f);
         Float_t const d0_XY =
            (TMath::Cos(angle) * fPhi->at(iHit) * fRadius->at(iHit)) + (TMath::Sin(angle) * hit.GetTimeStamp());
      ...
      if (fClusteredTBHits.size() > 0) {
         ...
         if (fClusteredTBHits_forw.size() > 0) {
            for (AtHit const &hitTB : fClusteredTBHits) {
               ...
               // std::cout << " Hit TB: " << TB << " Position X: " << positionTB.X() << " Position Y: " <<
               // positionTB.Y() << std::endl;
            }
            ...
            for (AtHit const &hitTB_forw : fClusteredTBHits_forw) {
               ...
               // std::cout << " Hit TB forw: " << TB_forw << " Position forw X: " << positionTB_forw.X() << " Position
               // forw Y: " << positionTB_forw.Y() << std::endl;
            }
            ...
            // std::cout << " Average Pos Forward X: " << posx_forw << " Average Pos Forward Y: " << posy_forw <<
            // std::endl;
         } // Hit forward if > 0
      }    // Hit if > 0

      Double_t distXY =
         TMath::Sqrt(((posx - posx_forw) * (posx - posx_forw)) + ((posy - posy_forw) * (posy - posy_forw)));
...
void AtHoughSpaceCircle::CalcHoughSpace(AtEvent *event, Bool_t YZplane, Bool_t XYplane, Bool_t XZplane)
{
   ...
   for (Int_t iHit = 0; iHit < (nHits - nstep); iHit++) {
      ...
      Double_t hitdist = TMath::Sqrt(TMath::Power(position.X() - position_next.X(), 2) +
                                     TMath::Power(position.Y() - position_next.Y(), 2) +
                                     TMath::Power(position.Z() - position_next.Z(), 2));
      // std::cout<<" Hit Coordinates - X : "<<position.X()<<" Y : "<<position.Y()<<" - Hit Coordinates Forward - X :
      // "<<position_forw.X()<<" Y : "<<position_forw.Y()<<std::endl; std::cout<<" Distance : "<<hitdist<<std::endl;
      ...
      for (Int_t itheta = 0; itheta < 1023; itheta++) {
         ...
         Float_t d0_XY_inv = 2.0 *
                             ((position.Y() - position_forw.Y()) * TMath::Sin(angle) +
                              (position.X() - position_forw.X()) * TMath::Cos(angle)) /
                             ((TMath::Power(position.Y(), 2) - TMath::Power(position_forw.Y(), 2)) +
                              (TMath::Power(position.X(), 2) - TMath::Power(position_forw.X(), 2)));
   ...
   for (Int_t iHit = 0; iHit < nHits - 1; iHit++) {
      ...
      Double_t dl = TMath::Sqrt(TMath::Power(position_forw.X() - position.X(), 2) +
                                TMath::Power(position_forw.Y() - position.Y(), 2));
      Double_t dz = (hit_forw.GetTimeStamp() - hit.GetTimeStamp()) * drift_cal;
      // fTheta->push_back(TMath::ATan2(dl,dz)); // Obsolete
   ...
   for (Int_t iHit = 0; iHit < nHits - 1; iHit++) {
      AtHit hit = event->GetHitArray()->at(iHit);
      TVector3 position = hit.GetPosition();
      Double_t geo_dist = TMath::Abs(TMath::Cos(fHoughLinePar.first) * fPhi->at(iHit) * fRadius->at(iHit) +
                                     TMath::Sin(fHoughLinePar.first) * hit.GetTimeStamp() - fHoughLinePar.second);
      Double_t hit_dist =
         TMath::Sqrt(TMath::Power((fPhi->at(iHit) * fRadius->at(iHit) - IniPhiBuff * IniRadiusBuff), 2) +
                     TMath::Power(hit.GetTimeStamp() - IniTSBuff, 2));
      ...
      if (geo_dist < 10.0) {
         ...
         if (hit_dist < 10.0) {
            ...
            // std::cout<<" Position Z : "<<position.Z()<<std::endl;
            // std::cout<<" Hit : "<<iHit<<" Timebucket : "<<hit.GetTimeStamp()<<" Geometrical distance : "<<geo_dist<<
            // " Hit Dist : "<<hit_dist<<std::endl;
         }
      }
   ...
   for (Int_t iHitTB = 0; iHitTB < clusterSize - 1; iHitTB++) {
      ...
      if (fClusteredTBHits.size() >
          0) { // Found at least 1 clustered hit in that time bucket, if not go to the next loop iteration
         // std::cout<<" ================================ "<<std::endl;
         // std::cout<<"  Starting cluster with TB : "<<fIniTS-iHitTB<<std::endl;
         fClusteredTBHits_forw = min->GetTBHitArray(fIniTS - iHitTB - 1 - TB_Scan, fClusteredHits);

         if (fClusteredTBHits_forw.size() ==
             0) { // If forward hit is empty scan until the end of the vector or until the next non-zero solution

            while (fClusteredTBHits_forw.size() == 0 &&
                   fIniTS - iHitTB - 1 - TB_Scan > 0) { // TODO: Use a lambda function for this mess
               TB_Scan++;
               fClusteredTBHits_forw = min->GetTBHitArray(fIniTS - iHitTB - 1 - TB_Scan, fClusteredHits);
               // std::cout<<TB_Scan<<std::endl;
               // std::cout<<fClusteredTBHits_forw.size()<<std::endl;
         ...
         if (fClusteredTBHits_forw.size() > 0) {
            ...
            if (fClusteredTBHits.size() > 0 && fClusteredTBHits_forw.size() > 0) {
               ...
               for (Int_t iHit = 0; iHit < fClusteredTBHits_forw.size(); iHit++) {
                  ...
                  cmsHits_Y_forw += positionTB_forw.Y() * hitTB_forw.GetCharge();
                  totCharge_forw += hitTB_forw.GetCharge();
                  // std::cout<<" Hit TB forw : "<<TB_forw<<" Position forw X : "<<positionTB_forw.X()<<" Position forw
                  // Y : "<<positionTB_forw.Y()<<std::endl;
               }
            }
   ...
   /*for(Int_t iHitT=0; iHitT<clusterSize-1;iHitT++ ){
    ...
    // AtHit hitT_forw = event->GetHitArray()->at(nHits-1-iHitT);
     ...
     TVector3 positionT_forw = hitT_forw.GetPosition();
           ...
           dl_theta+= TMath::Sqrt( TMath::Power(positionT_forw.X()-positionT.X(),2) +
   TMath::Power(positionT_forw.Y()-positionT.Y(),2)  ); fDl->push_back(dl_theta); fTheta->push_back(
   TMath::ATan2(dl_theta,(positionT_forw.Z() - IniPosZ ) ) ); // Distance in time between origin and Hit point (not
   HitT)
           //std::cout<<" Hit : "<<iHitT<<" Position Forw : "<<positionT_forw.Z()-IniPosZ<<" dl : "<<dl_theta<<" Time
   Stamp : "<<hitT.GetTimeStamp()<<std::endl;
...
void AtHoughSpaceCircle::CalcHoughSpace(AtEvent *event, TH2Poly *hPadPlane, const multiarray &PadCoord)
{
   ...
   /*
    ...
    std::function<Double_t(Double_t, std::vector<Double_t> &)> ELossFunc =
       std::bind(GetEloss, std::placeholders::_1, std::placeholders::_2);
    ...
    /////////////////////////// End of Hough Space Track Calculation   /////////////////////////////
       ...
       // This part of the code extracts the lines found in the RxPhi space using the RANSAC algorithm of the PCL
       // library. RANSAC returns the number of lines sorted by higher probabilty. The candidate line is found among
       // other lines by comparing the slope, intercept and distance between them. Once the line is selected, the
    initial
       // hit is chosen taking into account how many hits are found in a given TB and the density of hits along the
       // RANSAC line. Parameters to adjust: 1)  SetRANSACPointThreshold: Number of points to process (0.1 means that
    the
       // algorithm will stop when only 10% of the points are remaing) 2)  SetDistanceThreshold: Threshold distance to
       // accept inliers 3)  _Hits: Minimum number of hits per line found 4)  n_track_cnt: Maximum number of tracks
    found
       ...
       // 9)  fStdDevTB: Number of TB that contribute to the std deviation above a value (fixed by the user) in
    percetage
       ...
       if (trackSpiral.size() > 0) {
          for (Int_t ntrack = 0; ntrack < trackSpiral.size(); ntrack++) {
             ...
             if (_Hits > 5 &&
                 n_track_cnt < 5) { // Limited to 5 lines with more than 5 hits and spanning more than 2 Time Buckets
          ...
          if (trackSpiral_buff.size() > 0) {
             ...
             AtTrack &trackCand =
                FindCandidateTrack(trackSpiral_buff); // Find the most probable line and then scan points in the line
             ...
             while (lastHitMult == 0 && index < trackHits->size() && !kGoodDensity) {
                ...
                lastHitMult = GetTBMult(trackHits->at(index).GetTimeStamp(), trackHits,
                                        index); // Get the last RANSAC hit with TB mult >1
                if (lastHitMult) {
                   ...
                   // std::cout<<" Hit Time Bucket : "<<hit.GetTimeStamp()<<" lastHitMult "<<lastHitMult<<" hitdens
                   // "<<hitdens<<std::endl;
                   if (hitdens > fGoodDensity) {

                      kGoodDensity = kTRUE;
                      fIniHitRansac->SetHit(hit.GetHitPadNum(), hit.GetHitID(), position.X(), position.Y(),
    position.Z(), hit.GetCharge()); if (kDebug) std::cout << cGREEN << " Ini Hit TimeStamp : " << hit.GetTimeStamp() <<
    cNORMAL << std::endl; fIniHitRansac->SetTimeStamp(hit.GetTimeStamp()); std::vector<AtTrack *> circleTracks =
    smoothRadius(trackCand);
                      ...
                      if (circleTracks.size() > 0) {
                         ...
                         fIniRadiusRansac = TMath::Sqrt(TMath::Power((fXCenter - position.X()), 2) +
                                                        TMath::Power((fYCenter - position.Y()), 2));
                      std::cout << cRED << " Radius Ransac " << fIniRadiusRansac << "\n";
                      std::cout << " fXCenter " << fXCenter << "  fYCenter " << fYCenter << " " << position.X() << "  "
                                << position.Y() << cNORMAL << "\n";
    ...
    if (fRansacLinePar.size() > 0)
       RansacAngle = TMath::ATan2(fRansacLinePar.at(1), 1) * 180.0 / TMath::Pi();
...
Int_t AtHoughSpaceCircle::GetTBMult(Int_t TB, std::vector<AtHit> *harray, Int_t index)
{
   ...
   auto it =
      std::find_if(harray->begin() + index, harray->end(), [&TB](AtHit &hit) { return hit.GetTimeStamp() == TB; });
   if (it != harray->end()) {
...
AtTrack &AtHoughSpaceCircle::FindCandidateTrack(const std::vector<AtTrack *> &tracks)
{
   ...
   // The algorithm tries to get the line with the larger time bucket within a predifined value of the slope and
   // intercept
   ...
   if (tracks.size() > 1) {
      ...
      for (Int_t i = 1; i < tracks.size(); i++) {
         ...
         Double_t ydum_b =
            TMath::Sqrt(TMath::Power((fXCenter - pos_b_b.X()), 2) + TMath::Power((fYCenter - pos_b_b.Y()), 2)) *
            TMath::ATan2(fXCenter - pos_b_b.X(), fYCenter - pos_b_b.Y());
         Double_t xdum_f = hit_f_b.GetTimeStamp();
         Double_t ydum_f =
            TMath::Sqrt(TMath::Power((fXCenter - pos_f_b.X()), 2) + TMath::Power((fYCenter - pos_f_b.Y()), 2)) *
            TMath::ATan2(fXCenter - pos_f_b.X(), fYCenter - pos_f_b.Y());
         ...
         if (kDebug) {
            std::cout << " First hit of the first line : " << hit_b_b.GetTimeStamp()
                      << " Number of hits : " << trackHits_b->size() << std::endl;
            std::cout << " First hit of the second line : " << hit_f_b.GetTimeStamp()
                      << " Number of hits : " << trackHits_f->size() << std::endl;
            std::cout << " Last hit of the first line : " << hit_b_f.GetTimeStamp()
                      << " Number of hits : " << trackHits_b->size() << std::endl;
            std::cout << " Last hit of the second line : " << hit_f_f.GetTimeStamp()
                      << " Number of hits : " << trackHits_f->size() << std::endl;
            std::cout << " First line reduced Chi2 : " << chi2_b / ndf_b
                      << "    Second line reduced Chi2 : " << chi2_f / ndf_f << std::endl;
            std::cout << " First line std dev x : " << x_mean_b << " First line std dev y : " << y_mean_b << std::endl;
            std::cout << " Second line std dev x : " << x_mean_f << " Second line std dev y : " << y_mean_f
                      << std::endl;
            ...
            std::cout << " Length of the first line in TB : " << hit_b_f.GetTimeStamp() - hit_b_b.GetTimeStamp()
                      << std::endl;
            std::cout << " Length of the second line in TB : " << hit_f_f.GetTimeStamp() - hit_f_b.GetTimeStamp()
                      << std::endl;
         ...
         // Get the line on the right by comparing the last time bucket of each one and the slope. A maximum TB is
         // defined to keep the line with the latest point when comparing every  track just once. Then the standard
         // deviation of the points for each time bucket, the ratio between the number of points and length and the
         // number of time buckets with an unsual large std deviation are evaluated
         if ((hit_b_b.GetTimeStamp() > hit_f_b.GetTimeStamp()) && (LinePar_b.at(1) > fSlopeLimit) &&
             (hit_b_b.GetTimeStamp() > max_TB)) {
            if (x_mean_b < fStdDeviationLimit && y_mean_b < fStdDeviationLimit) // Mean std deviation upper limit
            {
               if (ratio_maxdev_b < fStdDevTB) { // The number of TB that contribute to the std dev limit (in
                                                 // percentage)
                  if (ratio_lp_b < fRatioLP &&
                      ratio_lp_b > 0) { // Ratio between the length in TB and the number of points
                     index = 0;
                     max_TB = hit_b_b.GetTimeStamp();
                  }
               }
            }
         } else if ((hit_b_b.GetTimeStamp() < hit_f_b.GetTimeStamp()) && (LinePar_f.at(1) > fSlopeLimit) &&
                    (hit_f_b.GetTimeStamp() > max_TB)) {
            if (x_mean_f < fStdDeviationLimit && y_mean_f < fStdDeviationLimit) // Mean std deviation upper limit
            {
               if (ratio_maxdev_f < fStdDevTB) { // The number of TB that contribute to the std dev limit (in
                                                 // percentage)
                  if (ratio_lp_f < fRatioLP &&
                      ratio_lp_f > 0) { // Ratio between the length in TB and the number of points
                     index = i;
...
void AtHoughSpaceCircle::GetTBDeviation(std::vector<AtHit> *hits, Float_t &maxdev_ratio, Double_t &mean_dev_x,
                                        Double_t &mean_dev_y)
{
      ...
      if (f_index < b_index) { // TODO: The problem comes when all the points have the same TB
         for (auto i = f_index; i <= b_index; i++) {
            ...
            if (hitArray.size() > 1) { // The deviation is only calculated for TB with more than 1 hit
               ...
               if (_x_dev > fStdDeviationLimit || _y_dev > fStdDeviationLimit)
                  maxdev_ratio++; // Number of time buckets with  std dev larger than a threshold
            ...
            if (kDebug)
               std::cout << cRED << " Hits : " << hitArray.size() << " TB : " << i << " _x_dev : " << _x_dev
                         << " _y_dev : " << _y_dev << cNORMAL << std::endl;
   ...
   if (kDebug)
      std::cout << cYELLOW << " Mean X dev : " << mean_dev_x << " Mean Y dev : " << mean_dev_y
                << " Ratio of TB with more than 5 " << maxdev_ratio << cNORMAL << std::endl;
}
...
Int_t AtHoughSpaceCircle::GetDensityOfHits(std::vector<AtHit> *hits, Int_t index, Int_t tb_range)
{
   ...
   int cnt = std::count_if(hits->begin(), hits->end(),
                           [&index, &tb_range](AtHit &hit) { return hit.GetTimeStamp() > index - tb_range; });
   return cnt;
}
...
std::vector<AtHit> AtHoughSpaceCircle::GetTBHitArray(Int_t TB, std::vector<AtHit> *harray)
{
   ...
   std::copy_if(harray->begin(), harray->end(), std::back_inserter(hitTBArray),
                [&TB](AtHit &hit) { return hit.GetTimeStamp() == TB; });
   return hitTBArray;
}

Double_t AtHoughSpaceCircle::GetEloss(Double_t c0, std::vector<Double_t> &par) //!!
{
   // TODO: This is not the generalized function! See AtTracking Analysis for a more general way. Here the parameters
   // come unsigned
   if (par.size() == 11) {
      // return
      // par[0]*(1./TMath::Power(c0,par[1]))*(1./(par[2]+par[3]/TMath::Power(c0,par[4])))+par[5]*TMath::Exp(-par[6]*TMath::Power((c0-par[7]),2));
      return par[0] * (1. / TMath::Power(c0, par[1])) * (1. / (par[2] + par[3] / TMath::Power(c0, par[4]))) +
             par[5] * TMath::Exp(par[6] * TMath::Power((c0 + par[7]), 2)) +
...
Double_t AtHoughSpaceCircle::GetXCenter() const
{
   return fXCenter;
}
Double_t AtHoughSpaceCircle::GetYCenter() const
{
   return fYCenter;
}
std::vector<Double_t> const *AtHoughSpaceCircle::GetRadiusDist() const
{
   return fRadius;
}
std::vector<Int_t> const *AtHoughSpaceCircle::GetTimeStamp() const
{
   return fTimeStamp;
}
std::vector<Double_t> const *AtHoughSpaceCircle::GetPhi() const
{
   return fPhi;
}
std::vector<Double_t> const *AtHoughSpaceCircle::GetTheta() const
{
   return fTheta;
}
std::vector<Double_t> const *AtHoughSpaceCircle::GetDl() const
{
   return fDl;
}

Double_t AtHoughSpaceCircle::GetIniPhi() const
{
   return fIniPhi;
}
Double_t AtHoughSpaceCircle::GetIniTheta() const
{
   return fIniTheta;
}
Double_t AtHoughSpaceCircle::GetIniRadius() const
{
   return fIniRadius;
}
Double_t AtHoughSpaceCircle::GetIniPhiRansac() const
{
   return fIniPhiRansac;
}
Double_t AtHoughSpaceCircle::GetIniThetaRansac() const
{
   return fIniThetaRansac;
}
Double_t AtHoughSpaceCircle::GetIniRadiusRansac() const
{
   return fIniRadiusRansac;
}

AtHit const *AtHoughSpaceCircle::GetIniHit() const
{
   return fIniHit;
}
AtHit const *AtHoughSpaceCircle::GetIniHitRansac() const
{
   return fIniHitRansac;
}

Double_t const *AtHoughSpaceCircle::GetInitialParameters() const
{
   return fParameter;
}
std::vector<double> const AtHoughSpaceCircle::GetInitialParametersVector() const
{
   return fParAux;
}

std::vector<Double_t> AtHoughSpaceCircle::GetPosXMin() const
{
   return fPosXmin;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosYMin() const
{
   return fPosYmin;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosZMin() const
{
   return fPosZmin;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosXExp() const
{
   return fPosXexp;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosYExp() const
{
   return fPosYexp;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosZExp() const
{
   return fPosZexp;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosXInt() const
{
   return fPosXinter;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosYInt() const
{
   return fPosYinter;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosZInt() const
{
   return fPosZinter;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosXBack() const
{
   return fPosXBack;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosYBack() const
{
   return fPosYBack;
}
std::vector<Double_t> AtHoughSpaceCircle::GetPosZBack() const
{
   return fPosZBack;
}
