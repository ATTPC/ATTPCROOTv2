...
void AtHoughSpaceCircle::CalcHoughSpace(AtEvent *event, TH2Poly *hPadPlane, const multiarray &PadCoord)
{
   ...
   /////////////////////////// End of Hough Space Track Calculation   /////////////////////////////
      ...
      // other lines by comparing the slope, intercept and distance between them. Once the line is selected, the initial
      // hit is chosen taking into account how many hits are found in a given TB and the density of hits along the
      // RANSAC line. Parameters to adjust: 1)  SetRANSACPointThreshold: Number of points to process (0.1 means that the
      // algorithm will stop when only 10% of the points are remaing) 2)  SetDistanceThreshold: Threshold distance to
      // accept inliers 3)  _Hits: Minimum number of hits per line found 4)  n_track_cnt: Maximum number of tracks found
            ...
            while (lastHitMult == 0 && index < trackHits->size() && !kGoodDensity) {
               ...
               if (lastHitMult) {
                  ...
                  if (hitdens > fGoodDensity) {
                     ...
                     kGoodDensity = kTRUE;
                     fIniHitRansac->SetHit(hit.GetHitPadNum(), hit.GetHitID(), position.X(), position.Y(), position.Z(),
                                           hit.GetCharge());
                     if (kDebug)
