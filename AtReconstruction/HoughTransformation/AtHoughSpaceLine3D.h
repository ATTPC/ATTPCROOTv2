...
#define GOLDEN_RAtIO 1.61803399
   ...
   static void lineTransform3D_fibonacciSphere(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                               std::vector<std::pair<vector3D, vector3D>> &lines,
                                               unsigned int threshold = 1, unsigned int n_directionalVectors = 20400,
                                               double step_plane = 1, unsigned int max_n = 0);

   static void lineTransform3D_azimuthElevation(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                                std::vector<std::pair<vector3D, vector3D>> &lines,
                                                unsigned int threshold = 1, double step_b = M_PI / 45,
                                                double step_plane = 1, unsigned int max_n = 0);

   static void lineTransform3D_Tesselation(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                           std::vector<std::pair<vector3D, vector3D>> &lines,
                                           unsigned int threshold = 1, unsigned char platonic_solid = HOUGH_ICOSAHEDRON,
                                           unsigned char granularity_b = 3, double step_plane = 20,
                                           unsigned int max_n = 10);

   static void
   lineTransform3D_Tesselation_weighted(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, std::vector<double> &weights,
                                        std::vector<std::pair<vector3D, vector3D>> &lines, unsigned int threshold = 1,
                                        unsigned char platonic_solid = HOUGH_ICOSAHEDRON,
                                        unsigned char granularity_b = 4, double step_plane = 1, unsigned int max_n = 0);

private:
   static void lineTransform3D(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                               std::vector<std::pair<vector3D, vector3D>> &lines, unsigned int threshold,
                               Sphere *sphere, double step_plane, unsigned int max_n);

   static void lineTransform3D_weighted(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, std::vector<double> &weights,
                                        std::vector<std::pair<vector3D, vector3D>> &lines, unsigned int threshold,
                                        Sphere *sphere, double step_plane, unsigned int max_n);

   static void nonmaximumSupression_AE(std::vector<unsigned int> &VotingSpace, Sphere *sphere, unsigned int num_b,
                                       unsigned int num_plane, int n, int k);

   static void nonmaximumSupression(std::vector<unsigned int> &VotingSpace, std::vector<vector3D> &vertices,
                                    std::deque<unsigned int> &triangles, unsigned int num_b, unsigned int num_plane,
                                    int n, int r = 1);

   static void nonmaximumSupression(std::vector<double> &VotingSpace, std::vector<vector3D> &vertices,
                                    std::deque<unsigned int> &triangles, unsigned int num_b, unsigned int num_plane,
                                    int n, int r = 1);

   static double roundToNearest(double num) { return (num > 0.0) ? floor(num + 0.5) : ceil(num - 0.5); }
