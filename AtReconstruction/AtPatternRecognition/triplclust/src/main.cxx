...
int main(int argc, char **argv)
{
   ...
   try {
      load_csv_file(infile_name, cloud_xyz, opt_params.get_delimiter(), opt_params.get_skip());
   } catch (const std::invalid_argument &e) {
      std::cerr << "[Error] in file'" << infile_name << "': " << e.what() << std::endl;
      return 2;
   }
#ifdef WEBDEMO
   // maximum pointcloud size error for webdemo
      ...
      std::cerr << "[Error] cannot read infile '" << infile_name << "'! " << e.what() << std::endl;
      return 2;
   }
   if (cloud_xyz.size() == 0) {
      std::cerr << "[Error] empty cloud in file '" << infile_name << "'" << std::endl
