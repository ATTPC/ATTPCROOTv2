...
public:
   ...
   bool operator()(const KdNode &p, const KdNode &q) { return (p.point[d] < q.point[d]); }
   size_t d;
};
...
KdTree::KdTree(const KdNodeVector *nodes, int _distance_type /*=2*/)
{
   size_t i, j;
   double val;
...
// distance_type can be 0 (Maximum), 1 (Manhatten), or 2 (Euklidean)
void KdTree::set_distance(int _distance_type, const DoubleVector *weights /*=NULL*/)
{
   if (distance)
      delete distance;
   this->distance_type = _distance_type;
   if (_distance_type == 0) {
      distance = (DistanceMeasure *)new DistanceL0(weights);
   } else if (_distance_type == 1) {
...
kdtree_node *KdTree::build_tree(size_t depth, size_t a, size_t b)
{
   ...
   if (b - a <= 1) {
      ...
      m = (a + b) / 2;
      std::nth_element(allnodes.begin() + a, allnodes.begin() + m, allnodes.begin() + b,
...
void KdTree::k_nearest_neighbors(const CoordPoint &point, size_t k, KdNodeVector *result,
                                 std::vector<double> *distances, KdNodePredicate *pred /*=NULL*/)
{
   size_t i;
   double d, temp_dist;
   KdNode temp;
   ...
   if (point.size() != dimension)
      throw std::invalid_argument("kdtree::k_nearest_neighbors(): point must be of same dimension as "
                                  "kdtree");
...
//--------------------------------------------------------------
bool KdTree::neighbor_search(const CoordPoint &point, kdtree_node *node, size_t k, SearchQueue *neighborheap)
{
   double curdist, dist;
...
//--------------------------------------------------------------
void KdTree::range_search(const CoordPoint &point, kdtree_node *node, double r, std::vector<size_t> *range_result)
{
