...
std::unique_ptr<AtPatternEvent> AtPATTERN::AtTrackFinderTC::FindTracks(AtEvent &event)
{
  ...
  opt_params.set_parameters(inputParams.s,inputParams.k,inputParams.n,inputParams.m,inputParams.r,inputParams.a,inputParams.t);
  ...
  std::cout<<" Cloud size : "<<cloud_xyz.size()<<" Clusters size : "<<cl_group.size()<<"\n";
...
std::unique_ptr<AtPatternEvent> AtPATTERN::AtTrackFinderTC::clustersToTrack(PointCloud& cloud, const std::vector<cluster_t> &clusters, AtEvent &event)
{
  ...
  for (size_t cluster_index = 0; cluster_index < clusters.size();++cluster_index) {
    ...
    for (std::vector<size_t>::const_iterator it = point_indices.begin();it != point_indices.end(); ++it) {
     ...
     ClusterizeSmooth3D(track, 15.0, 30.5);
