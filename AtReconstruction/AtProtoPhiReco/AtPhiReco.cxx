      ...
      if(nHits>1){
           ...
           Double_t Q_s = qhit_s.GetCharge();
               ...
               if(quadrant->GetQuadrantID()%2==1){
                  ...
                  if(PadNum_qf%2==1 && PadNum_qs%2==0) phi= (1.0 - (Q_f-Q_s)/(Q_f+Q_s)*2.0*a/da - (Q_f-Q_s)/(Q_f+Q_s)
)*45 ; else if(PadNum_qf%2==0 && PadNum_qs%2==1) phi = 90.0-phi; else{ std::cout<<"
======================================================================= "<<std::endl; std::cout<<" Warning, even-odd
sttagering not found. "<<std::endl;
                  }
                  ...
                  //std::cout<<" Inside an even quadrant ID : "<<quadrant->GetQuadrantID()<<std::endl;
                                                phi= (1.0 - (Q_f-Q_s)/(Q_f+Q_s)*2.0*a/da - (Q_f-Q_s)/(Q_f+Q_s) )*45 ;
                  if(PadNum_qf%2==0 && PadNum_qs%2==1) phi= (1.0 - (Q_f-Q_s)/(Q_f+Q_s)*2.0*a/da - (Q_f-Q_s)/(Q_f+Q_s)
)*45 ; else if(PadNum_qf%2==1 && PadNum_qs%2==0) phi = 90.0-phi; else{ std::cout<<"
======================================================================= "<<std::endl; std::cout<<" Warning, even-odd
sttagering not found. "<<std::endl;
                  }
