...
void AtPhiRecoTriple::PhiCalc(AtProtoQuadrant *quadrant, AtEvent *event)
{
   ...
   if (nHits > 2) {
      ...
      for (Int_t iHit = 1; iHit < nHits - 1;
           iHit++) { // Here we start from the second strip to make the average between both neighboring strips
         ...
         if (M_f == 2) {                                // Check if the first Pad in the vector has multihit
            if (M_s == 2 && (PadNum_qf == PadNum_qs)) { // Check if the next hit in the vector is the same pad and same
               ...
               if (T_f > T_s) {                         // Check which hit arrives first
                  // if(std::find(fast_HArray.begin(), fast_HArray.end(), qhit_f) != fast_HArray.end())
                  // fast_HArray.push_back(qhit_f); if(std::find(slow_HArray.begin(), slow_HArray.end(), qhit_s) !=
                  // slow_HArray.end()) slow_HArray.push_back(qhit_s);
                  fast_HArray.push_back(qhit_f);
                  slow_HArray.push_back(qhit_s);
               } else if (T_f < T_s) {
                  // if(std::find(fast_HArray.begin(), fast_HArray.end(), qhit_s) != fast_HArray.end())
                  // fast_HArray.push_back(qhit_s); if(std::find(slow_HArray.begin(), slow_HArray.end(), qhit_f) !=
                  // slow_HArray.end()) slow_HArray.push_back(qhit_f);
         ...
         if ((PadNum_qs == PadNum_qf + 1) && (PadNum_qt == PadNum_qs + 1)) {
            ...
            if (quadrant->GetQuadrantID() % 2 == 1) {
               ...
               /*TYPE B*/ else if (PadNum_qf % 2 == 0 && PadNum_qs % 2 == 1)
                  phi = (((Q_M - Q_s) / (Q_M + Q_s) * 2.0 * a / da + (Q_M - Q_s) / (Q_M + Q_s) - 1) *
                         45); // phi = 90.0-phi;
               else {
                  std::cout << " ======================================================================= " << std::endl;
         ...
         if (PadNum_qs == PadNum_qf + 1) {
            ...
            if (quadrant->GetQuadrantID() % 2 == 1) {
               ...
               /*TYPE A*/ if (PadNum_qf % 2 == 1 && PadNum_qs % 2 == 0)
                  phi = (1.0 - (Q_f - Q_s) / (Q_f + Q_s) * 2.0 * a / da - (Q_f - Q_s) / (Q_f + Q_s)) * 45;
               /*TYPE B*/ else if (PadNum_qf % 2 == 0 && PadNum_qs % 2 == 1)
                  phi = (((Q_f - Q_s) / (Q_f + Q_s) * 2.0 * a / da + (Q_f - Q_s) / (Q_f + Q_s) - 1) *
                         45); // phi = 90.0-phi;
               else {
                  std::cout << " ======================================================================= " << std::endl;
                  std::cout << " Warning, even-odd sttagering not found. " << std::endl;
