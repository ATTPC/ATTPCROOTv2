...
namespace AtRANSACN {
 ...
 public:
	...
	Double_t GetAngleTracks(const ROOT::Math::XYZVector & vec1,
				const ROOT::Math::XYZVector & vec2);
 ...
 protected:
	...
	void SetLine(double t, const double *p, double &x, double &y,
		     double &z);
	...
	friend inline std::ostream & operator <<(std::ostream & o,
						 const AtRANSACN::AtRansac::
						 PairedLines & pl) {

	    o << cGREEN << " =============================================== "
		<< std::endl;
	    o << " Lines ID : " << pl.LinesID.first << " - " << pl.LinesID.
		second << std::endl;
	    o << " Minimum distance between line : " << pl.minDist << std::endl;
	    o << " Mean vertex between line - X : " << pl.meanVertex.
		X() << "  - Y : " << pl.meanVertex.Y() << "  - Z : " << pl.
		meanVertex.Z() << std::endl;
	    o << " Angle with Z solenoid axis - Line 1 : " << pl.AngleZAxis.
		first << "  - Line 2 : " << pl.AngleZAxis.second << std::endl;
	    o << " Angle with Z detector axis - Line 1 : " << pl.AngleZDet.
		first << "  - Line 2 : " << pl.AngleZDet.second << std::endl;
	    o << " Angle with Y detector axis - Line 1 : " << pl.AngleYDet.
		first << "  - Line 2 : " << pl.AngleYDet.second << std::endl;
