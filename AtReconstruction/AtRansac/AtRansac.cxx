   ...
   if (tracksSize > 1) { // Defined in CalcGenHoughSpace
      for (Int_t ntrack = 0; ntrack < tracksSize; ntrack++) {
         ...
         if (nHits > fMinHitsLine) // We only accept lines with more than 5 hits and a maximum number of lines of 5
         {
            MinimizeTrack(&tracks->at(ntrack));
            tracks->at(ntrack).SetTrackID(ntrack);
            std::vector<Double_t> p = tracks->at(ntrack).GetPattern()->GetPatternPar();
            if (p.size() == 4) {
...
std::vector<AtTrack> *AtRANSACN::AtRansac::RansacPCL(const std::vector<AtHit> &hits)
{
   ...
   while (cloud->points.size() > fRANSACPointThreshold * nr_points) {
       ...
       */
         ...
         auto pattern = AtPatterns::CreatePattern(AtPatterns::PatternType::kLine);
         if (fRANSACModel == pcl::SACMODEL_CIRCLE2D)
            pattern = AtPatterns::CreatePattern(AtPatterns::PatternType::kCircle2D);
         pattern->SetPatternPar(coeff);
         track.SetPattern(std::move(pattern));
...
Int_t AtRANSACN::AtRansac::MinimizeTrack(AtTrack *track)
{
   ...
   auto line = AtPatterns::CreatePattern(AtPatterns::PatternType::kLine);
   line->SetPatternPar(parFit);
   line->SetChi2(Chi2_min);

   track->SetPattern(std::move(line));
...
void AtRANSACN::AtRansac::FindVertex(std::vector<AtTrack *> tracks)
{
   ...
   for (Int_t i = 0; i < int(tracks.size()) - 1; i++) {
      ...
      track->SetTrackID(i);
      std::vector<Double_t> p = track->GetPattern()->GetPatternPar();
      ...
      if (p.size() > 0) {
         ...
         for (Int_t j = i + 1; j < tracks.size(); j++) {
            ...
            std::vector<Double_t> p_f = track->GetPattern()->GetPatternPar();
