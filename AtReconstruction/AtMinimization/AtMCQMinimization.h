...
public:
   ...
   bool MinimizeGen(Double_t *parameter, T *event, const std::function<std::vector<R> *()> &func, TH2Poly *hPadPlane,
...
protected:
   ...
   Double_t
   GetChi2Pos(Int_t index, Int_t _iterCorrNorm, Int_t _par, Double_t *_xTBCorr, Double_t *_yTBCorr, Double_t *_zTBCorr);
   template <typename T>
   Double_t GetChi2Range(T *event, std::vector<Double_t> &_xTBCorr, std::vector<Double_t> &_yTBCorr,
                         std::vector<Double_t> &_zTBCorr, Double_t sigma, Int_t npoints);
   ...
   void QMCsim(double *parameter, double *Qsim, double *zsimq, double &QMCtotal, double x0MC, double y0MC, double z0MC,
               double phiMCv, double aMCv, double Bminv, double densv, double rominv, double &e0sm, multiarray PadCoord,
               TH2Poly *padplane);
   ...
   Bool_t kIsZGeoVertex; // Uses the relative Z vertex determined with the calibration performed with the original TB
                         // taken from parameter list
...
template <typename T, typename R>
bool AtMCQMinimization::MinimizeGen(Double_t *parameter, T *event, const std::function<std::vector<R> *()> &func,
                                    TH2Poly *hPadPlane, const multiarray &PadCoord)
{
   ...
   if (fELossPar_array[0].size() > 0 && fRtoEPar_array[0].size() > 0)
      ...
      std::cerr << cRED << " AtMCQMinimization::MinimizeOptMapAmp - Function parameters not found ! " << cNORMAL
                << std::endl;
   ...
   MCvar(parameter, icontrol, iconvar, x0MC, y0MC, z0MC, aMC, phiMC, Bmin, fDens, romin, x0MCv, y0MCv, z0MCv, aMCv,
   ...
   for (imc1 = 0; imc1 < imc1max; imc1++) {
      ...
      for (imc2 = 0; imc2 < imc2max; imc2++) {
         icontrol = 2;
         // fPadPlane->Reset(0);
         MCvar(parameter, icontrol, iconvar, x0MC, y0MC, z0MC, aMC, phiMC, Bmin, fDens, romin, x0MCv, y0MCv, z0MCv,
               aMCv, phiMCv, Bminv, densv, rominv); // for MC variation with same starting value as before
         ...
         Chi2MC(Qtrack, ztrackq, Qtracktotal, Qsim, zsimq, QMCtotal, Chi2Q, sigmaq,
                sigmaz); // Chi2 to compare track and MC
         // Chi2Q = CHi2fit;

         // TODO: Warning! Check parameter[7] in the AtHoughSpaceCircle line 1109
         if (kPosChi2)
            Chi2fitPos = GetChi2Pos(imc1, fIterCorrNorm, parameter[7], fXTBCorr, fYTBCorr, fZTBCorr);
         if (kRangeChi2)
            Chi2Range = GetChi2Range(event, fPosXmin, fPosYmin, fPosZmin, sigmaz, 0);
         ...
         if (kRangeChi2)
            CHi2fit = (Chi2Q + Chi2Range) / 2.0;
         else
            CHi2fit = (Chi2Q + Chi2fitPos) / 2.0;
         ...
         if (CHi2fit < Chimin) {
            ...
            chi2minQ = Chi2Q;
            Chimin = CHi2fit;
            // std::cout<<cRED<<" Iteration : "<<imc1<<"    "<<imc2<<" Chi2Fit : "<<CHi2fit<<" Chi2Q : "<<Chi2Q<<"
            // Chi2Range : "<<Chi2Range<<cNORMAL<<std::endl;
            icontrol = 3;
            MCvar(parameter, icontrol, iconvar, x0MC, y0MC, z0MC, aMC, phiMC, Bmin, fDens, romin, x0MCv, y0MCv, z0MCv,
   ...
   QMCsim(parameter, Qsim, zsimq, QMCtotal, x0MC, y0MC, z0MC, aMC, phiMC, Bmin, fDens, romin, e0sm, PadCoord,
   ...
   // Chi2MC(Qtrack,ztrackq,Qtracktotal,Qsim,zsimq,QMCtotal,Chi2Q,sigmaq,sigmaz);   //Chi2 to compare track and MC for
   // Chimin parameters if(kRangeChi2) Chi2Range = GetChi2Range(event,fPosXmin,fPosYmin,fPosZmin,sigmaz,0);
...
template <typename T>
Double_t AtMCQMinimization::GetChi2Range(T *event, std::vector<Double_t> &_xTBCorr, std::vector<Double_t> &_yTBCorr,
                                         std::vector<Double_t> &_zTBCorr, Double_t sigma, Int_t npoints)
{
   ...
   chi2 = TMath::Sqrt(TMath::Power(posEp.X() - _xTBCorr.back(), 2) + TMath::Power(posEp.Y() - _yTBCorr.back(), 2) +
                      TMath::Power(posEp.Z() - _zTBCorr.back(), 2));
