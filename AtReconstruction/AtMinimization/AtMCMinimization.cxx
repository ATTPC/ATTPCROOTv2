...
std::vector<Double_t> AtMCMinimization::GetPosYMin()
{
   return fPosYmin;
}
std::vector<Double_t> AtMCMinimization::GetPosZMin()
{
   return fPosZmin;
}
std::vector<Double_t> AtMCMinimization::GetPosXExp()
{
   return fPosXexp;
}
std::vector<Double_t> AtMCMinimization::GetPosYExp()
{
   return fPosYexp;
}
std::vector<Double_t> AtMCMinimization::GetPosZExp()
{
   return fPosZexp;
}
std::vector<Double_t> AtMCMinimization::GetPosXInt()
{
   return fPosXinter;
}
std::vector<Double_t> AtMCMinimization::GetPosYInt()
{
   return fPosYinter;
}
std::vector<Double_t> AtMCMinimization::GetPosZInt()
{
   return fPosZinter;
}
std::vector<Double_t> AtMCMinimization::GetPosXBack()
{
   return fPosXBack;
}
std::vector<Double_t> AtMCMinimization::GetPosYBack()
{
   return fPosYBack;
}
std::vector<Double_t> AtMCMinimization::GetPosZBack()
{
   return fPosZBack;
}
   ...
   if (parameter[7] < 500) {
      ...
      for (Int_t i = 0; i < 5; i++) {
         ...
         for (Int_t j = 0; j < 400; j++) // MC
         {
            ...
            Double_t dens = 0.06363 * 18 * (1 + step8 * (gRandom->Rndm() - 0.5)) /
                            20.; //  !DENSITY ISOBUTANE At 20 TORR corrected 18 torr
            ...
            for (Int_t k = 0; k < iterationmax; k++) {
               ...
               if (iz1 == 1) {
                  ...
                  if (m == 2)
                     c0 = c0 / 2.0;
                  sloss = 6.98 * (1. / TMath::Power(c0, 0.83)) * (1. / (20. + 1.6 / TMath::Power(c0, 1.3))) +
                          0.2 * TMath::Exp(-30. * TMath::Power((c0 - 0.1), 2)); // Old expression
                  // sloss = 0.3*TMath::Power((1./c0),0.78)*(1./(1.+0.023/TMath::Power(c0,1.37)));
               }
               if (iz1 == 6) {
                  c0 = ekin / 6.;
                  sloss = 36. * (1. / TMath::Power(c0, 0.83)) * (1. / (1.6 + 1.6 / TMath::Power(c0, 1.5))) +
                          1. * TMath::Exp(TMath::Power(-(c0 - 0.5), 2));
               }
               if (iz1 == 2) {
                  c0 = ekin;
               ...
               Double_t vcin =
                  sqrt(TMath::Power(dxdt, 2) + TMath::Power(dydt, 2) + TMath::Power(dzdt, 2)); // !v in cm/ns
            ...
            for (Int_t iChi = 0; iChi < imaxchi2; iChi++) {
               ...
               // if(iChi<parameter[7])   hitTBArray=GetTBHitArray(parameter[3]-iChi,fHitArray); // Seach for Hits with
               // the same TB
               // if(event->GetEventID()==973) std::cout<<" iChi : "<<iChi<<" parameters 3 "<<parameter[3]<<" parameters
               // 7 "<<parameter[7]<<" Hit TB Size "<<hitTBArray.size()<<std::endl;

               // NB: Simulated data progress in forward direction in the sd::vector while the experimental one is
               //  the opposite
               //                                                   Initial Time Bucket
               //                                                           |
               //                                                        Simulation -> Increasing time bucket
               //                                                        (std::vector position)
               ...
               if (kDebug) {
                  std::cout
                     ...
                     << "  ------------------------------------------------------------------------------------------- "
                     << cNORMAL << std::endl;
                  std::cout << " Array : " << iChi << " X_Sim : " << xcmm[iChi] << " Y_Sim : " << ycmm[iChi]
                            << " Z_Sim : " << zcmm[iChi] << " TB Sim" << floor(zcmm[iChi] / dzstep / 10) << std::endl;
                  std::cout << cGREEN << " Array Corr: " << iChi << " X_Sim_Corr : " << xTBCorr[iChi]
                            << " Y_Sim_Corr : " << yTBCorr[iChi] << " Z_Sim_Corr : " << zTBCorr[iChi] << cNORMAL
                            << std::endl;
                  ...
                  // std::cout<<iChi<<" X_exp : "<<position.X()<<" Y_exp : "<<position.Y()<<" Hit TS :
                  // "<<parameter[3]-HitTS<<std::endl;
                  std::cout << " Number of hits with same TB : " << hitTBArray.size() << std::endl;
                  std::cout << " Experimental TB " << parameter[3] - iChi << std::endl;
               }

               if (xTBCorr[iChi] == (Double_t)-10000)
                  kIsSim = kFALSE;
               else
                  kIsSim = kTRUE;

               // std::cout<<" Experimental Time Bucket : "<<parameter[3]-iChi<<"  Simulated Time Bucket : "<<round(
               // (zcmm[iChi]- fZk/10)/dzstep + parameter[3] )<<std::endl;
               ...
               if (hitTBArray.size() > 0) {
                  ...
                  for (Int_t iHitTB = 0; iHitTB < hitTBArray.size();
                     ...
                     // Double_t hitDist = TMath::Sqrt( TMath::Power(positionTB.X()-xcmm[iChi],2) +
                     // TMath::Power(positionTB.Y()-ycmm[iChi],2) );
                     ...
                     // TO compare Sim and Exp uncomment this
                     if (kDebug)
                        std::cout << iChi << " X_exp : " << positionTB.X() << " Y_exp : " << positionTB.Y()
                                  << " Z_exp : " << posz << " Hit TS : " << hitTB.GetTimeStamp() << std::endl;
                  ...
                  // TO compare Sim and Exp uncomment this
                  if (kDebug)
                     std::cout << cGREEN << " Average X : " << posx << " Average Y : " << posy
                               << " Average Z : " << posz << cNORMAL << std::endl;
               ...
               if (kIsExp) {
                  ...
                  if (i > 2 && chi2_buff > 10.0)
                     chi2 += 10.0;
                  else if (i < 3 && chi2_buff > 100.0)
                     chi2 += 100.0;
                  else
                     chi2 += chi2_buff;
                  chi2buff[iChi] = chi2_buff;
                  if (kDebug)
                     std::cout << " Point chi Square : " << chi2_buff
                               << " for Experimental Time bucket : " << parameter[3] - iChi << std::endl;
                  if (kDebug)
                     std::cout << " Total chi Square : " << chi2 << std::endl;
                  fPosXinter = xinter;
                  fPosYinter = yinter;
                  fPosZinter = zinter;
                  fPosTBinter = TBInter;
               ...
               if (kDebug && !kIsSim)
                  std::cout << cRED << " Point without simulated value! " << cNORMAL << std::endl;
               if (kDebug && !kIsExp)
                  std::cout << cRED << " Point without experimental value! " << cNORMAL << std::endl;
               if (kDebug)
                  std::cout
                     << cRED
                     << "  ------------------------------------------------------------------------------------------- "
            ...
            if (chi2 < chi2min) {
               ...
               if (kVerbose) {
                  std::cout << cYELLOW << " New Min chi2 : " << chi2min << " for MC iteration : " << j
                            << " of the step " << i << std::endl;
            ...
            for (Int_t iChi = 0; iChi < imaxchi2; iChi++) {
               ...
               if (kDebug) {
                  std::cout
                     ...
                     << "  ------------------------------------------------------------------------------------------- "
                     << cNORMAL << std::endl;
                  std::cout << " Array : " << iChi << " X_Sim : " << xcmm << " Y_Sim : " << ycmm << " Z_Sim : " << zcmm
                            << " TB Sim" << floor(zcmm / dzstep / 10) << std::endl;
                  std::cout << cGREEN << " Array Corr: " << iChi << " X_Sim_Corr : " << xTBCorr[iChi]
                            << " Y_Sim_Corr : " << yTBCorr[iChi] << " Z_Sim_Corr : " << zTBCorr[iChi] << cNORMAL
                            << std::endl;
                  std::cout << " Real Time bucket : " << parameter[3] + iChi << std::endl;
