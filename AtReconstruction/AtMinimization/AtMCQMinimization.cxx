   ...
   AtMCQMinimization::AtMCQMinimization()
{
   ...
   esm = z1 * 1.75879e-3 * 0.510998918 /
         restmass; // ![e/m electron cm**2/(Volt*nsec**2] this is not the energy/mass but charge/mass
...
std::vector<Double_t> AtMCQMinimization::GetPosYMin()
{
   return fPosYmin;
}
std::vector<Double_t> AtMCQMinimization::GetPosZMin()
{
   return fPosZmin;
}
std::vector<Double_t> AtMCQMinimization::GetPosXExp()
{
   return fPosXexp;
}
std::vector<Double_t> AtMCQMinimization::GetPosYExp()
{
   return fPosYexp;
}
std::vector<Double_t> AtMCQMinimization::GetPosZExp()
{
   return fPosZexp;
}
std::vector<Double_t> AtMCQMinimization::GetPosXInt()
{
   return fPosXinter;
}
std::vector<Double_t> AtMCQMinimization::GetPosYInt()
{
   return fPosYinter;
}
std::vector<Double_t> AtMCQMinimization::GetPosZInt()
{
   return fPosZinter;
}
std::vector<Double_t> AtMCQMinimization::GetPosXBack()
{
   return fPosXBack;
}
std::vector<Double_t> AtMCQMinimization::GetPosYBack()
{
   return fPosYBack;
}
std::vector<Double_t> AtMCQMinimization::GetPosZBack()
{
   return fPosZBack;
}

void AtMCQMinimization::AddELossFunc(std::function<Double_t(Double_t, std::vector<Double_t> &)> &func)
{
   fEloss_func_array.push_back(func);
}
void AtMCQMinimization::AddRtoEFunc(std::function<Double_t(Double_t, std::vector<Double_t> &)> &func)
{
   fRtoE_func_array.push_back(func);
}
std::vector<std::function<Double_t(Double_t, std::vector<Double_t> &)>> *AtMCQMinimization::GetELossFunctionArray()
{
   return &fEloss_func_array;
}
void AtMCQMinimization::AddELossPar(std::vector<Double_t> (&par)[10])
{
   for (auto i = 0; i < 10; i++)
      fELossPar_array[i] = par[i];
}
void AtMCQMinimization::AddRtoEPar(std::vector<Double_t> (&par)[10])
{
   for (auto i = 0; i < 10; i++)
      fRtoEPar_array[i] = par[i];
}
void AtMCQMinimization::AddParticle(std::vector<std::pair<Int_t, Int_t>> &ptcl)
{
   fParticleAZ = ptcl;
}
void AtMCQMinimization::SetEntTB(Int_t value)
{
   fEntTB = value;
}
void AtMCQMinimization::SetZGeoVertex(Bool_t value)
{
   kIsZGeoVertex = value;
}
void AtMCQMinimization::SetEntZ0(Double_t val)
{
   fEntZ0 = val;
}
void AtMCQMinimization::SetBackWardPropagation(Bool_t value)
{
   kBackWardProp = value;
}
void AtMCQMinimization::SetGainCalibration(Double_t value)
{
   fGain = value;
}
void AtMCQMinimization::SetLongDiffCoef(Double_t value)
{
   fCoefL = value;
}
void AtMCQMinimization::SetTranDiffCoef(Double_t value)
{
   fCoefT = value;
}
void AtMCQMinimization::SetStepParameters(Double_t (&par)[10])
{
   for (auto i = 0; i < 10; i++)
      fStep_par[i] = par[i];
}
void AtMCQMinimization::SetRangeChi2(Bool_t value)
{
   kRangeChi2 = value;
}
                                            ...
                                            const multiarray &PadCoord)
{
   ...
   if (fParticleAZ.size() > 0) {
      ...
      std::cerr << cRED
                << " AtMCQMinimization::MinimizeOptMapAmp -  Warning ! Particle (A,Z) not found. Using A : " << m
                << "  -  Z : " << iz1 << cNORMAL << std::endl;
   ...
   for (Int_t i = 0; i < fHitArray->size(); i++) {
      ...
      // std::cout<<cGREEN<<" Hit number : "<<i<<" X : "<<position.X()<<" Y : "<<position.Y()<<" Z : "<<position.Z()<<"
      // TB : "<<hitTB<<" Pad Number : "<<hitPadNum<<" charge "<<hitcharge<<cNORMAL<<std::endl;
   ...
   // std::fout1<<" Hit number : "<<i<<" X : "<<position.X()<<" Y : "<<position.Y()<<" Z : "<<position.Z()<<" TB :
   // "<<hitTB<<" Pad Number : "<<hitPadNum<<" charge "<<hitcharge<<cNORMAL<<std::endl;
 ...
 for (size_t i = 0; i < npadtotal; i++) {
   ...
   MCvar(parameter, icontrol, iconvar, x0MC, y0MC, z0MC, aMC, phiMC, Bmin, dens, romin, x0MCv, y0MCv, z0MCv, aMCv,
         phiMCv, Bminv, densv, rominv); // for initialisation
   ...
   for (imc1 = 0; imc1 < imc1max; imc1++) {
      ...
      for (imc2 = 0; imc2 < imc2max; imc2++) {
         ...
         // fPadPlane->Reset(0);
         MCvar(parameter, icontrol, iconvar, x0MC, y0MC, z0MC, aMC, phiMC, Bmin, dens, romin, x0MCv, y0MCv, z0MCv, aMCv,
               phiMCv, Bminv, densv, rominv); // for MC variation with same starting value as before
         QMCsim(parameter, Qsim, zsimq, QMCtotal, x0MCv, y0MCv, z0MCv, aMCv, phiMCv, Bminv, densv, rominv, e0sm,
                PadCoord, fPadPlane);
         // std::cout<<cRED<<" After QMCsim x "<<x0MCv<<" y "<< y0MCv<< " z "<< z0MCv<<" theta "<< aMCv<<" phi "<<
         // phiMCv<<" B " <<Bminv<<" dens "<< densv<< " e0sm "<<e0sm<<" ro "<<rominv<<cNORMAL;
         Chi2MC(Qtrack, ztrackq, Qtracktotal, Qsim, zsimq, QMCtotal, CHi2fit, sigmaq,
                sigmaz); // Chi2 to compare track and MC
         ...
         if (CHi2fit < Chimin) {
            ...
            icontrol = 3;
            MCvar(parameter, icontrol, iconvar, x0MC, y0MC, z0MC, aMC, phiMC, Bmin, dens, romin, x0MCv, y0MCv, z0MCv,
                  aMCv, phiMCv, Bminv, densv, rominv);
         }
   ...
   QMCsim(parameter, Qsim, zsimq, QMCtotal, x0MC, y0MC, z0MC, aMC, phiMC, Bmin, dens, romin, e0sm, PadCoord,
          fPadPlane); // simulation with Chimin parameters
   ...
   // std::cout<<cRED<<" final fit x "<<x0MC <<" y "<< y0MC << " z "<< z0MC <<" theta "<< aMC <<" phi "<< phiMC <<" B "
   // <<Bmin <<" dens "<< dens <<" e0sm "<<e0sm<<" ro "<<romin <<cNORMAL<<std::endl;
   Chi2MC(Qtrack, ztrackq, Qtracktotal, Qsim, zsimq, QMCtotal, CHi2fit, sigmaq,
          sigmaz); // Chi2 to compare track and MC for Chimin parameters
   ...
   std::cout << " Vertex Position (Backward extrapolation) - X : " << fVertexPos.X() << " - Y : " << fVertexPos.Y()
             << " - Z : " << fVertexPos.Z() << std::endl;
...
void AtMCQMinimization::MCvar(double *parameter, int &modevar, int &iconvar, double &x0MC, double &y0MC, double &z0MC,
                              double &aMC, double &phiMC, double &Bmin, double &dens, double &romin, double &x0MCv,
                              double &y0MCv, double &z0MCv, double &aMCv, double &phiMCv, double &Bminv, double &densv,
                              double &rominv)
{
   ...
   if (modevar == 1) { // initiatialisation
      ...
      /*                                  std::cout << "indside mc modevar "<< modevar << "  iconvar "
         <<iconvar<<std::endl; std::cout << "indside mc modevar " <<modevar<< std::endl; std::cout << "indside mc x0 "
         <<x0MCv<< "  "<<x0MC << std::endl; std::cout << "indside mc y " <<y0MCv<<std::endl; std::cout << "indside z"
         <<z0MCv<<std::endl; std::cout << "indside mc a " <<aMCv<< std::endl; std::cout << "indside mc  phi " <<
         phiMCv<<std::endl; */
   }
   if (modevar == 2) { // MC variation
      ...
      z0MCv =
         z0MC +
         step6 * (0.5 - gRandom->Rndm()); //  std::cout<<" first loop"<<iterd<<" i "<<i<<"  j  "<<j << std::endl; //wm
...
void AtMCQMinimization::QMCsim(double *parameter, double *Qsim, double *zsimq, double &QMCtotal, double x0MC,
                               double y0MC, double z0MC, double aMC, double phiMC, double Bmin, double dens,
                               double romin, double &e0sm, multiarray PadCoord, TH2Poly *padplane)
{
   ...
   if (B > 0) {
      ...
      if (fRtoE_func_array.size() > 0) {
         ...
         std::cout << cYELLOW << " AtMCQMinimization::QMCsim - Warning! No Range-to-Energy function found." << cNORMAL
                   << std::endl;
   }
   ...
   for (Int_t k = 0; k < integrationmax; k++) {
      ...
      if (!kBackWardProp)
         fBeam_range = fEntZ0 - zpad[0];
      if (kIsZGeoVertex)
         zpad[iterd] = zpad[iterd] - (fZk - fEntZ0);
      ...
      if (k == 0)
         iterCorr_0 = iterCorr; // Offset renomarlization
      ...
      if (fEloss_func_array.size() > 0) {
         ...
         std::cout << cYELLOW << " AtMCQMinimization::QMCsim - Warning! No energy loss function found." << cNORMAL
                   << std::endl;
      ...
      /*if(iz1==1){
         c0=ekin;
         if(m==2) c0=c0/2.0;
         sloss=6.98*(1./TMath::Power(c0,0.83))*(1./(20.+1.6/TMath::Power(c0,1.3)))+0.45*TMath::Exp(-55.*TMath::Power((c0-0.025),2));
      // expression modified June 14 2016
         //sloss=6.98*(1./TMath::Power(c0,0.83))*(1./(20.+1.6/TMath::Power(c0,1.3)))+1.0*TMath::Exp(-55.*TMath::Power((c0-0.025),2));
      // expression modified June 14 2016 low energy term changed
         //
      ...
      if(iz1==2){
         c0=ekin;
         sloss=11.95*(1./TMath::Power(c0,0.83))*(1./(2.5+1.6/TMath::Power(c0,1.5)))+
      0.05*TMath::Exp(TMath::Power(-(c0-0.5),2));
       }*/
   ...
   for (iterd = 0; iterd < integration; iterd++) {
      ...
      for (isig = 0; isig < 4; ++isig) {
         ...
         for (iphi = 0; iphi < 8; ++iphi) {
            ...
            // nota bene: we do not for the moment calculate the longitudinal straggling because only center of gravity
            // used
   ...
   for (Int_t i = 0; i < npadtotal; i++) {
      ...
      if (Qsim[i] > tracksimthreshold) // threshold after renormalization
      {
         ...
         Float_t xhelp =
            (PadCoord[i][0][0] + PadCoord[i][1][0] + PadCoord[i][2][0]) / 3.; // x center of gravity of the pad
         Float_t yhelp =
            (PadCoord[i][0][1] + PadCoord[i][1][1] + PadCoord[i][2][1]) / 3.; // y center of gravity of the pad
         ivalidsim++;
         double help;
         help = zsimq[i];
...
void AtMCQMinimization::BackwardExtrapolation()
{
   ...
   for (Int_t i = 0; i < 200; i++) {
      ...
      if (i == 0)
         iterCorr_0 = iterCorr; // Offset renomalization
      iterCorrNorm = -iterCorr_0 + iterCorr;
      ...
      if (i == 199)
         if (kVerbose)
            std::cout << cYELLOW << " Warning: Minimum distance of approach not found ! " << cNORMAL << std::endl;
   }
}

void AtMCQMinimization::Chi2MC(double *Qtrack, double *ztrackq, double &Qtracktotal, double *Qsim, double *zsimq,
                               double &QMCtotal, double &Chi2fit, double &sigmaq, double &sigmaz)
{
   ...
   // std:: cout << "npoints  "<<npoints<<"  Qtrack  " <<  Qtrackval << " Qsim "<<  Qsimval<< "  ratio  "
   // <<ratioq<<endl;
   //  chi2 calculation
   Chi2fit = 0.;
   double Chi2fitq = 0.;
   ...
   if (npoints > 0)
      Chi2fit = (Chi2fitq + Chi2fitz) / float(2 * npoints);
   if (npoints > 0)
      Chi2fit = Chi2fit / float(2 * npoints); /// for z and Q and npoints
   if (npoints < 5)
      Chi2fit = 10000.; /// to avoid chi2 by too little number of points

   // std:: cout<< " chi2fitnorm**3  "<<Chi2fit<<"  Chi2q  " << Chi2fitq<<"  Chi2z " << Chi2fitz<<" Chi2number "<<
   // Chi2number<<" npoints"<< npoints <<endl;
   // std:: cout<< " chi2fitnorm  "<<Chi2fit<<"  Chi2q  " << Chi2fitq<<"  Chi2z " << Chi2fitz<<" npoints "<< npoints
   // <<endl;
}
...
TVector3 AtMCQMinimization::TransformIniPos(Double_t x, Double_t y, Double_t z)
{
   ...
   Double_t z_sol = z_det*TMath::Cos(fTiltAng) - y_det*TMath::Sin(fTiltAng) +
   fZk/10.0*TMath::Power(TMath::Sin(fTiltAng),2); Double_t y_sol = ( y_det - (fZk/10.0-z_sol)*TMath::Sin(fTiltAng) )/
   TMath::Cos(fTiltAng);
   ...
   Double_t x_sol = x_det;
   Double_t z_sol = (z_det * TMath::Cos(fTiltAng) + y_det * TMath::Sin(fTiltAng) +
                     fZk / 10.0 * TMath::Power(TMath::Sin(fTiltAng), 2));
...
Double_t AtMCQMinimization::GetChi2Pos(Int_t index, Int_t _iterCorrNorm, Int_t _par, Double_t *_xTBCorr,
                                       Double_t *_yTBCorr, Double_t *_zTBCorr)
{
   ...
   for (Int_t iChi = 0; iChi < imaxchi2; iChi++) {
      ...
      std::vector<AtHit> hitTBArray;
      if (hitTBMatrix->size() > iChi)
         hitTBArray = hitTBMatrix->at(iChi);

      if (_xTBCorr[iChi] == (Double_t)-10000)
         kIsSim = kFALSE;
      else
         kIsSim = kTRUE;
      ...
      if (hitTBArray.size() > 0) {
         ...
         for (Int_t iHitTB = 0; iHitTB < hitTBArray.size();
              iHitTB++) { // TODO: There no need to do this every chisquare loop
            ...
            // if(kDebug)
            // std::cout<<iChi<<" X_exp : "<<positionTB.X()<<" Y_exp : "<<positionTB.Y()<<" Z_exp : "<<posz<<" Hit TS :
            // "<<hitTB.GetTimeStamp()<<std::endl;
      ...
      if (kIsExp) {
         ...
         if (index > 2 && chi2_buff > 10.0)
            chi2 += 10.0;
         else if (index < 3 && chi2_buff > 100.0)
            chi2 += 100.0;
         else
            chi2 += chi2_buff;
         // chi2buff[iChi]=chi2_buff;
         // if(kDebug) std::cout<<" Point chi Square : "<<chi2_buff<<" for Experimental Time bucket :
         // "<<iChi<<std::endl; if(kDebug) std::cout<<" Total chi Square : "<<chi2<<std::endl; fPosXinter=xinter;
         // fPosYinter=yinter;
         // fPosZinter=zinter;
         // fPosTBinter=TBInter;
         num_MC_Point++;
