...
UInt_t GETLayerHeader::GetHeaderSize(Bool_t inBytes)
{
   return CorrectEndianness(fHeaderSize, 2) * (inBytes ? GetUnitBlock() : 1);
}
UInt_t GETLayerHeader::GetItemSize()
{
   return CorrectEndianness(fItemSize, 2);
}
UInt_t GETLayerHeader::GetNItems()
{
   return CorrectEndianness(fNItems, 4);
}
UInt_t GETLayerHeader::GetEventID()
{
   return CorrectEndianness(fEventID, 4);
}
ULong64_t GETLayerHeader::GetEventTime()
{
   return CorrectEndianness(fEventTime, 6);
}
UInt_t GETLayerHeader::GetDeltaT()
{
   return CorrectEndianness(fDeltaT, 2);
}
...
{
   switch (GetFrameType()) {
   case GETFRAMEMERGEDBYID: return GetHeaderSize() - GETLAYERHEADERBYIDSIZE;

   case GETFRAMEMERGEDBYTIME: return GetHeaderSize() - GETLAYERHEADERBYTIMESIZE;
   }
...
void GETLayerHeader::Read(ifstream &stream)
{
   ...
   switch (GetFrameType()) {
   case GETFRAMEMERGEDBYID: stream.read((Char_t *)fEventID, 4); break;
...
void GETLayerHeader::Print()
{
   ...
   cout << "   frameSize: " << GetFrameSize(kFALSE) << " (" << dec << GetFrameSize(kFALSE)
        << " Blocks = " << GetFrameSize() << hex << " Bytes)" << endl;
   cout << "  dataSource: " << GetDataSource() << endl;
   cout << "   frameType: " << GetFrameType() << endl;
   cout << "    revision: " << GetRevision() << endl;
   cout << "  headerSize: " << GetHeaderSize(kFALSE) << " (" << dec << GetHeaderSize(kFALSE)
        << " Blocks = " << GetHeaderSize() << hex << " Bytes)" << endl;
   cout << "    itemSize: " << GetItemSize() << endl;
   cout << "      nItems: " << GetNItems() << dec << " (" << GetNItems() << ")" << hex << endl;
   switch (GetFrameType()) {
   case GETFRAMEMERGEDBYID: cout << "     eventID: " << dec << GetEventID() << endl; break;
