...
typedef std::function<float(triplet const &lhs, triplet const &rhs, pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud)>
   TripletMetric;
typedef std::function<float(cluster const &lhs, cluster const &rhs, Eigen::MatrixXf const &d,
                            pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud)>
   ClusterMetric;

inline float
defaultTripletMetric(triplet const &lhs, triplet const &rhs, pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud)
{
   ...
   std::cout << perpendicularDistanceA << ", " << perpendicularDistanceB << ", "
             << std::max(perpendicularDistanceA, perpendicularDistanceB) << ',' << std::pow(2.0f, 1.0f + 12.0f * angle)
             << std::endl;
...
cluster_history CalculateHc(pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud, std::vector<triplet> const &triplets,
                            ClusterMetric clusterMetric = singleLinkClusterMetric,
                            TripletMetric tripletMetric = defaultTripletMetric);
...
std::vector<AtTrajectory> ToTrajectories(pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud,
                                         std::vector<AtHit> const &hits, std::vector<triplet> const &triplets,
                                         cluster_group const &clusterGroup, float const splineTangentScale,
                                         float const splineMinControlPointDistance, size_t const splineJump,
