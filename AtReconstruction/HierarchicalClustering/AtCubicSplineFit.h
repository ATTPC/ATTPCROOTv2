...
public:
   ...
   AtCubicSplineFit(std::vector<Eigen::Vector3f> const &controlPoints, float tangentScale = 0.5f,
                    float minControlPointDistance = 0.01f, size_t jump = 1,
                    PositionFunction positionFunction = defaultPositionFunction);
   ...
   float CalculateAverageCurvature(float const startPosition, float const endPosition, size_t const sampleSize,
                                   float const delta = 0.1f) const;
