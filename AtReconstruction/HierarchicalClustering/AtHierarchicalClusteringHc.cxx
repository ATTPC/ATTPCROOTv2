...
static std::vector<Eigen::Vector3f> PointCloud2Vectors(pcl::PointCloud<T> const &cloud)
{
   ...
   for (T const &pclPoint : cloud) {
      result.push_back(Eigen::Vector3f(pclPoint.x, pclPoint.y, pclPoint.z));
   }
...
std::vector<triplet>
GenerateTriplets(pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud, size_t nnKandidates, size_t nBest, float maxError)
{
   ...
   for (size_t pointIndexB = 0; pointIndexB < cloud->size(); ++pointIndexB) {
      ...
      int const nnFound =
         kdtree.nearestKSearch(*cloud, (int)pointIndexB, (int)nnKandidates, nnIndices, nnbSquaredDistances);
...
static Eigen::MatrixXf CalculateDistanceMatrix(pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud,
                                               std::vector<triplet> const &triplets, TripletMetric tripletMetric)
{
...
cluster_history CalculateHc(pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud, std::vector<triplet> const &triplets,
                            ClusterMetric clusterMetric, TripletMetric tripletMetric)
{
   ...
   while (currentGeneration.clusters.size() > 1) {
      ...
      for (size_t i = 0; i < currentGeneration.clusters.size(); ++i) {
         for (size_t j = i + 1; j < currentGeneration.clusters.size(); ++j) {
            float const clusterDistance =
               clusterMetric(currentGeneration.clusters[i], currentGeneration.clusters[j], distanceMatrix, cloud);
      ...
      merged.insert(merged.end(), currentGeneration.clusters[bestClusterPair.second].cbegin(),
...
cluster_group CleanupClusterGroup(cluster_group const &clusterGroup, size_t minTriplets)
{
   ...
   auto newEnd =
      std::copy_if(clusterGroup.clusters.cbegin(), clusterGroup.clusters.cend(), cleanedGroup.clusters.begin(),
                   [&](cluster const &clusterEl) { return clusterEl.size() >= minTriplets; });
...
ToTrajectories(pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud, std::vector<AtHit> const &hits,
               std::vector<triplet> const &triplets, cluster_group const &clusterGroup, float const splineTangentScale,
               float const splineMinControlPointDistance, size_t const splineJump, std::vector<AtHit> *noMatch)
{
   ...
   for (auto const &currentCluster : clusterGroup.clusters) {
      try {
         ...
         *clusterCloud = ExtactAtIndices(*cloud, pointIndices);
         ...
         // std::vector<AtHierarchicalClusteringGraph::state> const mstHistory =
         // AtHierarchicalClusteringGraph::CalculateMinimumSpanningTree(*clusterCloud);
         // std::vector<AtHierarchicalClusteringGraph::edge> const edges = mstHistory.back().edges;
         // Eigen::MatrixXf allPairsShortestPath = AtHierarchicalClusteringGraph::CalculateAllPairsShortestPath(edges,
         // clusterCloud->size());
         ...
         // make sure mainDirection points from startHit to endHit
         {
            ...
            if (AtTrajectory::GetPositionOnMainDirection(
                   centroidPoint, mainDirection, Eigen::Vector3f(startHitPos.X(), startHitPos.Y(), startHitPos.Z())) >
                AtTrajectory::GetPositionOnMainDirection(centroidPoint, mainDirection,
                                                         Eigen::Vector3f(endHitPos.X(), endHitPos.Y(), endHitPos.Z())))
               mainDirection *= -1.0f;
         }

         AtCubicSplineFit const cubicSplineFit(
            PointCloud2Vectors(*clusterCloud), splineTangentScale, splineMinControlPointDistance, splineJump,
            [&](Eigen::Vector3f const &point, size_t index) {
         ...
         std::stable_sort(sortedClusterHits.begin(), sortedClusterHits.end(), [&](AtHit const &lhs, AtHit const &rhs) {
            ...
            return AtTrajectory::GetPositionOnMainDirection(centroidPoint, mainDirection,
                                                            Eigen::Vector3f(lhsPos.X(), lhsPos.Y(), lhsPos.Z())) <
                   AtTrajectory::GetPositionOnMainDirection(centroidPoint, mainDirection,
                                                            Eigen::Vector3f(rhsPos.X(), rhsPos.Y(), rhsPos.Z()));
         });
