...
//  0  0  1  0
//  1  0  0  0
static float const hermitricMatrixValue[] = {2.0f, -3.0f, 0.0f, 1.0f, -2.0f, 3.0f,  0.0f, 0.0f,
                                             1.0f, -2.0f, 1.0f, 0.0f, 1.0f,  -1.0f, 0.0f, 0.0f};
Eigen::Matrix4f const AtCubicSplineFit::hermitricMatrix(hermitricMatrixValue);

AtCubicSplineFit::AtCubicSplineFit(std::vector<Eigen::Vector3f> const &controlPoints, float tangentScale,
                                   float minControlPointDistance, size_t jump,
                                   AtCubicSplineFit::PositionFunction positionFunction)
   : _controlPoints(controlPoints), _positionFunction(positionFunction)
{
   this->_tangentScale = tangentScale;

   std::stable_sort(this->_controlPoints.begin(), this->_controlPoints.end(),
                    [&](Eigen::Vector3f const &lhs, Eigen::Vector3f const &rhs) {
...
Eigen::Vector3f AtCubicSplineFit::CalculatePoint(float position) const
{
   ...
   if (position < this->GetStartPosition()) {
      ...
      float const scaledPosition =
         (position - firstSplineSegment.p0Pos) / (firstSplineSegment.p1Pos - firstSplineSegment.p0Pos);
      ...
      float const scaledPosition =
         (position - lastSplineSegment.p1Pos) / (lastSplineSegment.p1Pos - lastSplineSegment.p0Pos);
   ...
   auto splineSegmentIt = std::lower_bound(spline.cbegin(), spline.cend(), position,
                                           [](SplineSegment const &lhs, float rhs) { return lhs.p1Pos < rhs; });
   ...
   std::cout << "P: " << position << " (" << scaledPosition << ") "
       ...
       << "(" << controlPoints[splineSegment.p0Index](0) << " " << controlPoints[splineSegment.p0Index](1) << " " <<
   controlPoints[splineSegment.p0Index](2) << ") "
       << splineSegment.p1Index << " "
       << "(" << controlPoints[splineSegment.p1Index](0) << " " << controlPoints[splineSegment.p1Index](1) << " " <<
   controlPoints[splineSegment.p1Index](2) << ") "
       << "(" << splineSegment.m0(0) << " " << splineSegment.m0(1) << " " << splineSegment.m0(2) << ") "
       << "(" << splineSegment.m1(0) << " " << splineSegment.m1(1) << " " << splineSegment.m1(2) << ")"
       << std::endl;
   */

   Eigen::RowVector4f const tVector(scaledPosition * scaledPosition * scaledPosition, scaledPosition * scaledPosition,
                                    scaledPosition, 1);
...
{
   return (this->CalculateDerivativePoint(position + delta, delta) -
           this->CalculateDerivativePoint(position - delta, delta)) /
          (delta + delta);
}

float AtCubicSplineFit::CalculateAverageCurvature(float const startPosition, float const endPosition,
                                                  size_t const sampleSize, float const delta) const
{
   float result = 0.0f;
