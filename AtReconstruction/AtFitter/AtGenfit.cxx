   ...
   AtFITTER::AtGenfit::AtGenfit(Float_t magfield, Float_t minbrho, Float_t maxbrho, Int_t minit, Int_t maxit)
{
   ...
   db->AddParticle("Triton", "Triton",2.80943211, kFALSE, khShGev / (12.33 * kYear2Sec), 3, "Ion",
                   1000010030);
   db->AddParticle("Alpha", "Alpha",3.7284, kTRUE, khShGev / (12.33 * kYear2Sec), 6, "Ion",
                   1000020040);
   ...
   if(fVerbosity>0)
	   {
         std::cout << " Initial angles from PRA "
                   << "\n";
	 std::cout << " Theta : " << TMath::RadToDeg()*track->GetGeoTheta() << " - Phi : " << TMath::RadToDeg()*track->GetGeoPhi() << "\n";
	   }
...
if(track->GetGeoTheta()>90.0*TMath::DegToRad()){
	 ...
	 TVector3 posSeed(iniPos.X() / 10.0, iniPos.Y() / 10.0, zIniCal / 10.0);
	 ...
	 TVector3 momSeed(px,py,pz);
	   ...
	   if(fVerbosity>0)fitState.Print();
	   // Fit result
            fitState.getPosMomCov(pos_res, mom_res, cov_res);
	    if(fVerbosity>0) std::cout << cYELLOW << " Total Momentum : " << mom_res.Mag() << " - Position : " << pos_res.X() << "  "
