...
void GETFrame::SetNumTbs(Int_t value)
{
   fNumTbs = value;
}
...
void GETFrame::SetPolarity(Int_t value)
{
   fPolarity = value;
}
   ...
   if (fIsCalcPedestalUsed[index / 512] && fIsSetPedestalUsed[index / 512]) {
      if (fPolarity < 0)
         return fPedestalData[index + buckIdx] - rmsFactor * fPedestalSigmaData[index + buckIdx] -
                (fPedestalDataMean[index / 512] - fInternalPedestal[index / 512]);
      else
         return fPedestalData[index + buckIdx] + rmsFactor * fPedestalSigmaData[index + buckIdx] -
                (fPedestalDataMean[index / 512] - fInternalPedestal[index / 512]);
