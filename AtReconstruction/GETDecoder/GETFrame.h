...
public:
   ...
   //! Set the raw ADC **value** of the specific bucket, **buckIdx**, of the channel, **chIdx**, in the AGET,
   //! **agetIdx**.
   ...
   //! Return the raw ADC values array with the number of time buckets specified in GETConfig of the channel, **chIdx**,
   //! in the AGET, **agetIdx**.
   Int_t *GetRawADC(Int_t agetIdx, Int_t chIdx);
   //! Return the ADC value of the specific time bucket, **buckIdx**, of the channel, **chIdx**, in the AGET,
   //! **agetIdx**.
   ...
   //! Return the pedestal-subtracted ADC values array with the number of time buckets specified in GETConfig of the
   //! channel, **chIdx**, in the AGET, **agetIdx**.
   Double_t *GetADC(Int_t agetIdx, Int_t chIdx);
   //! Return the pedestal-subtracted ADC value of the specific time bucket, **buckIdx**, of the channel, **chIdx**, in
   //! the AGET, **agetIdx**.
   Double_t GetADC(Int_t agetIdx, Int_t chIdx, Int_t buckIdx);
   //! Return the pedestal value of time bucket, **buckIdx**, and channel, **chIdx**, in the AGET, **agetIdx**. Sigma
   //! value is multiplied by **rmsFactor** before returning.
   Double_t GetPedestal(Int_t agetIdx, Int_t chIdx, Int_t buckIdx, Double_t rmsFactor = 0);
...
private:
   ...
   Double_t fFPNSigmaThreshold;      /// Sigma threshold when calculating baseline value
