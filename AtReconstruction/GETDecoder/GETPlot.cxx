...
TCanvas *GETPlot::ShowSummarySpectra(Int_t startTb, Int_t numTbs, Int_t notConnected, Int_t innerFrame)
{
   ...
   while ((fFrame = fDecoder->GetFrame(-1, innerFrame))) {
      for (Int_t iAget = 0; iAget < 4; iAget++) {
...
TCanvas *
GETPlot::ShowFrame(Int_t frameNo, Int_t innerFrameNo, Int_t startTb, Int_t numTbs, Int_t numChannels, Int_t *chList)
{
   /**
    * \par Usage
...
TCanvas *GETPlot::PrintMax(Int_t eventNo, Int_t innerFrameNo, Int_t startTb, Int_t numTbs)
{
   ...
   for (Int_t iAget = 0; iAget < 4; iAget++) {
      for (Int_t iCh = 0; iCh < 68; iCh++) {
         ...
         std::cout << std::setw(10) << fFrame->GetPedestal(iAget, iCh, 0) - fFrame->GetRawADC(iAget, iCh, maxADCIdx)
                   << std::endl;
      }
   }
