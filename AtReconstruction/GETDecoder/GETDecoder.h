...
public:
   ...
   //! Return the number of merged frames. Note that this number only valid after the first frame of merged frame is
   //! loaded by GetFrame() method.
...
private:
   ...
   Int_t
      fMergedHeaderSize; /// header size of merged frame. For additional skip bytes when finding frame by frame number.
   Int_t
      fNumMergedFrames; /// the number of merged frames. For additional skip bytes when finding frame by frame number.
   Int_t fUnitBlock;    /// Unit block size used in frame header
   ULong64_t fMergedFrameStartPoint;  /// byte number of the merged frame start point. For navigational feature in a
                                      /// merged frame.
   ULong64_t fCurrentMergedFrameSize; /// size of a merged frame of the frame ID **fCurrentFrameID**. For additional
                                      /// skip bytes when finding frame by frame number.
