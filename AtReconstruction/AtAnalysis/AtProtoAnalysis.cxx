...
void AtProtoAnalysis::SetHoughDist(Double_t value)
{
   fHoughDist = value;
}
void AtProtoAnalysis::SetUpperLimit(Double_t value)
{
   fUpperLimit = value;
}
void AtProtoAnalysis::SetLowerLimit(Double_t value)
{
   fLowerLimit = value;
}

void AtProtoAnalysis::Analyze(AtProtoEvent *protoevent, AtProtoEventAna *protoeventAna, AtHoughSpaceLine *houghspace,
                              TF1 *(&HoughFit)[4], TGraph *(&HitPatternFilter)[4], TF1 *(&FitResult)[4])
{
   ...
   for (Int_t i = 0; i < 4; i++) {
      ...
      for (Int_t j = 0; j < qNumHit; j++) {
         ...
         // Double_t distance = TMath::Sqrt( TMath::Power(position.X(),2) + TMath::Power(position.Y(),2) +
         // TMath::Power(position.Z(),2));
         Double_t distance =
            TMath::Sqrt(TMath::Power(position.X(), 2) + TMath::Power(position.Y(), 2) + TMath::Power(posZCalCorr, 2));
         ...
         // Double_t geo_dist = TMath::Abs (TMath::Cos(fHoughPar.at(i).first)*radius  +
         // TMath::Sin(fHoughPar.at(i).first)*position.Z()  - fHoughPar.at(i).second);
         Double_t geo_dist = TMath::Abs(TMath::Cos(fHoughPar.at(i).first) * radius +
                                        TMath::Sin(fHoughPar.at(i).first) * posZCalCorr - fHoughPar.at(i).second);
         // if(geo_dist<fHoughDist) HitPatternFilter[i]->SetPoint(HitPatternFilter[i]->GetN(),radius,position.Z());
         if (geo_dist < fHoughDist)
            HitPatternFilter[i]->SetPoint(HitPatternFilter[i]->GetN(), radius, posZCalCorr);
      ...
      if (HitPatternFilter[i]->GetN() > 3) { // Minimum number of points to perform the fit
         ...
         if (FitResult[i]) {
            ...
            if (par1 >= 0)
               afit = TMath::Pi() - TMath::ATan2(1, TMath::Abs(par1));
            else if (par1 < 0)
               afit = TMath::ATan2(1, TMath::Abs(par1));
   ...
   // Vertex matching
   if (TMath::Abs(fPar0_fit.at(0) - fPar0_fit.at(2)) < fVertexDiff)
      protoeventAna->SetVertex02((fPar0_fit.at(0) + fPar0_fit.at(2)) / 2.0);
   if (TMath::Abs(fPar0_fit.at(1) - fPar0_fit.at(3)) < fVertexDiff)
      protoeventAna->SetVertex13((fPar0_fit.at(1) - fPar0_fit.at(3)) / 2.0);
}
