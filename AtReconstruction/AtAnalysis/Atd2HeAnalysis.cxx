...
void Atd2HeAnalysis::SetThetaCM(Double_t value)
{
   THcm = value;
}
void Atd2HeAnalysis::SetMissingMass(Double_t value)
{
   MissingMass = value;
}

Double_t Atd2HeAnalysis::GetThetaCM()
{
   return THcm;
}
Double_t Atd2HeAnalysis::GetMissingMass()
{
   return MissingMass;
}
Double_t Atd2HeAnalysis::GetThetaFit()
{
   return theta_fit;
}
Double_t Atd2HeAnalysis::GetPhiFit()
{
   return phi_fit;
}
bool Atd2HeAnalysis::GetFitStatus()
{
   return ok;
}
Double_t Atd2HeAnalysis::GetVertexFit()
{
   return vertex;
}

Double_t Atd2HeAnalysis::omega(Double_t x, Double_t y, Double_t z)
{
...
void Atd2HeAnalysis::kine_2b(Double_t m1, Double_t m2, Double_t m3, Double_t m4, Double_t K_proj, Double_t thetalab,
                             Double_t K_eject)
{
   ...
   m4_ex = sqrt((cos(thetalab) * Atd2HeAnalysis::omega(s, pow(m1, 2), pow(m2, 2)) *
                    Atd2HeAnalysis::omega(u, pow(m2, 2), pow(m3, 2)) -
   ...
   theta_cm =
      ...
      acos((pow(s, 2) + s * (2 * t - pow(m1, 2) - pow(m2, 2) - pow(m3, 2) - pow(m4_ex, 2)) +
           ...
           (Atd2HeAnalysis::omega(s, pow(m1, 2), pow(m2, 2)) * Atd2HeAnalysis::omega(s, pow(m3, 2), pow(m4_ex, 2))));
...
void Atd2HeAnalysis::FitTrack(TGraph2D *f2dtrack, TGraph *fpadtrack, std::vector<Double_t> *iniguess)
{
   ...
   if (vertex > iniguess->at(5))
      theta_fit = 180.0 - theta_fit;

   TFitResultPtr res1 = fpadtrack->Fit("pol1", "Sq");
   phi_fit = atan2(signo(iniguess->at(4)) * signo(res1->Value(1)) * fabs(res1->Value(1)), signo(iniguess->at(4))) *
             TMath::RadToDeg();
   if (phi_fit < 0)
      phi_fit += 360;
}
...
TMatrixD Atd2HeAnalysis::Calcd(TMatrixD alpha0)
{
  ...
  */
   ...
   // double h5 = sq(alpha0[3][0]) - p1.Mag2() - sq(m1) /*+ sq(alpha0[10][0]) - p3.Mag2() - sq(m3) + sq(alpha0[17][0]) -
   // p4.Mag2() - sq(m4)*/  ; double h6 = alpha0[7][0]*alpha0[7][0] + alpha0[8][0]*alpha0[8][0]  +
   // alpha0[9][0]*alpha0[9][0] + sq(m3) - sq(alpha0[10][0]);
...
void Atd2HeAnalysis::KFit(std::vector<double> pin, std::vector<double> &pout)
{
   ...
   alpha1[1][0] = pin[1];
   alpha1[2][0] = pin[2];
   alpha1[3][0] = pin[3]; // projectile (px,py,pz,E)
   alpha2[0][0] = pin[4];
   alpha2[1][0] = pin[5];
   alpha2[2][0] = pin[6];
   alpha2[3][0] = pin[7]; // ejectile (px,py,pz,E)
   alpha3[0][0] = pin[8];
   alpha3[1][0] = pin[9];
   alpha3[2][0] = pin[10];
   alpha3[3][0] = pin[11]; // proton 1 (px,py,pz,E)
   alpha4[0][0] = pin[12];
   alpha4[1][0] = pin[13];
   alpha4[2][0] = pin[14];
   alpha4[3][0] = pin[15]; // proton 2 (px,py,pz,E)
