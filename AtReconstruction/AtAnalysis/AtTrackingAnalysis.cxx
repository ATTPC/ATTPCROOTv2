...
void AtTrackingAnalysis::SetElossParameters(std::vector<Double_t> (&parE)[10])
{
   for (Int_t i = 0; i < 10; i++)
      fElossPar[i] = parE[i];
}
void AtTrackingAnalysis::SetEtoRParameters(std::vector<Double_t> (&parRtoE)[10])
{
   for (Int_t i = 0; i < 10; i++)
      fEtoRPar[i] = parRtoE[i];
}
void AtTrackingAnalysis::AddParticle(std::vector<std::pair<Int_t, Int_t>> &ptcl)
{
   fParticleAZ = ptcl;
}

void AtTrackingAnalysis::Analyze(AtPatternEvent *patternEvent, AtTrackingEventAna *trackingEventAna, TH2Poly *hPadPlane,
                                 const multiarray &PadCoord)
{
   if (fElossPar[0].size() > 0 && fParticleAZ.size() > 0) {
      ...
      auto init =
         std::initializer_list<Double_t>({8.0, 8.0, 0.1, 0.1, 0.1, 0.1, 0.0, 0.0, 0.1, 0.0}); // MonteCarlo steps
      ...
      std::function<Double_t(Double_t, std::vector<Double_t> &)> RtoEFunc =
         std::bind(GetEnergyFromRange, std::placeholders::_1, std::placeholders::_2);
      ...
      for (Int_t i = 0; i < trackCand.size(); ++i) {
         ...
         trackToMin->SetPosMin(min->GetPosXMin(), min->GetPosYMin(), min->GetPosZMin(), min->GetPosXBack(),
                               min->GetPosYBack(), min->GetPosZBack());
         trackToMin->SetPosExp(min->GetPosXExp(), min->GetPosYExp(), min->GetPosZExp(), min->GetPosXInt(),
                               min->GetPosYInt(), min->GetPosZInt());
      ...
      std::cout << cYELLOW << " AtTrackingAnalysis::Analyze - Warning! No Energy Loss or Particle parameters found! "
                << cNORMAL << std::endl;
   }
}

void AtTrackingAnalysis::Analyze(AtRANSACN::AtRansac *Ransac, AtTrackingEventAna *trackingEventAna, TH2Poly *hPadPlane,
                                 const multiarray &PadCoord)
{
   ...
   // The algorithm will try to fit the track according to the possible particles added by the user from the
   // macro/database. For the moment the ELoss function is taken from this class but eventually the user can define
   ...
   if (fElossPar[0].size() > 0 && fParticleAZ.size() > 0) {
      ...
      if (trackCand.size() > 1) { // At least two tracks to form a vertex
         ...
         for (Int_t i = 0; i < 2; i++) {
            Int_t trackID = 0;
            if (i == 0)
               trackID = pairTrackIndex.first;
            else if (i == 1)
               trackID = pairTrackIndex.second;
            ...
            if (indexID > -1 && indexID < trackCand.size()) {
               ...
               if (trackAngle > (fTiltAng + 1.0)) {
                  // std::cout<<" Accepted Line : "<<track->GetTrackID()<<" with angle :
                  // "<<track->GetAngleZAxis()<<std::endl;
         ...
         if (mininizationTracks.size() < 2) {
            ...
            for (Int_t i = 0; i < trackCand.size(); i++) {
               if (i != pairTrackIndex.first && i != pairTrackIndex.second) {
                  ...
                  Double_t dist = TMath::Sqrt(TMath::Power((trackVertex.X() - Vertex_mean.X()), 2) +
                                              TMath::Power((trackVertex.Y() - Vertex_mean.Y()), 2) +
                                              TMath::Power((trackVertex.Z() - Vertex_mean.Z()), 2));

                  // std::cout<<" Evaluating Line : "<<track->GetTrackID()<<" with angle :
                  // "<<track->GetAngleZAxis()<<std::endl; std::cout<<" Distance from vertex : "<<dist<<std::endl;

                  if (trackAngle > (fTiltAng + 1.0) && dist < 6.0) {
                     // 3std::cout<<" Accepted Line : "<<track->GetTrackID()<<" with angle :
                     // "<<track->GetAngleZAxis()<<std::endl;
                     track->SetGeoRange(track->GetLinearRange(Vertex_mean));
                     Double_t ener = RtoEFunc(track->GetLinearRange(Vertex_mean) * (fPressure / 760.0), fEtoRPar[0]);
                     track->SetGeoEnergy(ener);
      ...
      // Calculation of the entrance time bucket and rotation of the detector frame to find the angles in the solenoid
      // frame.
      Double_t Z0 = Vertex_mean.Z() + TMath::Sqrt(TMath::Power(Vertex_mean.X(), 2) + TMath::Power(Vertex_mean.Y(), 2)) /
                                         TMath::Tan(fTiltAng * TMath::Pi() / 180.0);
      ...
      fVertexEnergy = RtoEFunc(fVertex * (fPressure / 760.0),
                               fEtoRPar[0]); // Default case: 4He+4He. Every function and parameter list is the same.
      min->SetEntTB(fEntTB_calc);
      min->SetZGeoVertex(
         kTRUE); // If true, the MC will start where the geometrical vertex is found,
...
void AtTrackingAnalysis::AnalyzeSimple(AtRANSACN::AtRansac *Ransac, AtTrackingEventAna *trackingEventAna,
                                       TH2Poly *hPadPlane, const multiarray &PadCoord)
{
...
Double_t AtTrackingAnalysis::GetEnergyFromRange(Double_t range, std::vector<Double_t> &par)
{
   ...
   if (par.size() == 7) {
      return par[0] * TMath::Sqrt(range) + par[1] * (1.0 + par[2] / (TMath::Sqrt(range) + par[3])) +
             par[4] / (TMath::Power(TMath::Sqrt(range) + par[5], 2) + par[6]);
      ...
      std::cerr << cRED << " AtHoughSpaceCircle::GetEnergyFromRange -  Warning ! Wrong number of parameters."
                << std::endl;
   return 0;
}
...
{
   // This function returns the original time according to the original Z calculation that takes fEntTb (and no
   // fEntTB_calc)
...
std::pair<Double_t, Double_t> AtTrackingAnalysis::GetAnglesSolenoid(AtTrack *track)
{
   ...
   */
   ...
   if (par1 >= 0)
      afit = TMath::Pi() - TMath::ATan2(TMath::Abs(par1), 1);
   else if (par1 < 0)
      afit = TMath::ATan2(TMath::Abs(par1), 1);
