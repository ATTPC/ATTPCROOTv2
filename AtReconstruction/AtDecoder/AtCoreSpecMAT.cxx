...
AtCoreSpecMAT::AtCoreSpecMAT(Int_t opt)
    : AtPadCoordArr(boost::extents[3174][3][2]), kOpt(0) {
  kOpt = opt;
  Initialize();
  SetNumTbs(512);
...
AtCoreSpecMAT::AtCoreSpecMAT(TString filename, Int_t opt)
    : AtPadCoordArr(boost::extents[3174][3][2]), kOpt(0) {
...
AtCoreSpecMAT::AtCoreSpecMAT(TString filename, Int_t numTbs, Int_t windowNumTbs,
                             Int_t windowStartTb)
    : AtPadCoordArr(boost::extents[3174][3][2]), kOpt(0) {
  Initialize();
  AddData(filename);
...
Bool_t AtCoreSpecMAT::SetInhibitMaps(TString inimap, TString lowgmap,
                                     TString xtalkmap) {
  fIniMap = inimap;
  fLowgMap = lowgmap;
  fXtalkMap = xtalkmap;
...
void AtCoreSpecMAT::GetFPNChannelsFromROOTFILE(Long64_t eventID) {
  ...
  std::vector<int> ChannelsFPNpp = {11, 22, 45, 56}; // fpn channels for
                                                     // padplane
  std::vector<int> ChannelsFPNsc = {43, 44, 46,
                                    47}; // fpn channels for scintillators
  ...
  while (myReader.Next() && (!EventCompleted)) {
    if (*myInternalEventNr == eventID) {
      ...
      for (Int_t i = 0; i < 4; i++) { // loop over number of fpn channels
        ...
        } else if ((PadRef[3] == ChannelsFPNsc[i]) &&
                   !(fIsPadPlaneCobo[PadRef[0]])) {
          Nr_fpn_found++;
          // std::cout << "Added fpnchannel" << std::endl;
          // std::cout << ", corresponding to Cobo: " << PadRef[0] << "   Asad:
          // "  << PadRef[1] << "   Aget: "  << PadRef[2] << "   Ch: "  <<
  ...
  std::cout << "A total of " << Nr_fpn_found
  ...
  while (myReader.Next() && (!EventCompleted)) {
    ...
    if (*myInternalEventNr == eventID) {
      ...
      if (PadRefNum != -1 && !IsInhibited) {
        ...
        Bool_t good = fPedestalPtr[0]->SubtractPedestal(
            fNumTbs, fpn_adc, rawadc, adc, 5, fIsNegativePolarity[PadRef[0]], 5,
            20);
...
Bool_t AtCoreSpecMAT::SetWriteFile(TString filename, Int_t coboIdx,
                                   Bool_t overwrite) {
...
void AtCoreSpecMAT::WriteData() {
  if (fRawEventPtr == NULL) {
    std::cout << "== [AtCore] Call this method after GetRawEvent()!"
              << std::endl;
...
AtRawEvent *AtCoreSpecMAT::GetRawEvent(Long64_t eventID) {
  ...
  if (!fIsData) {
    std::cout << cRED << "== [AtCore] Data file is not set!" << cNORMAL
              << std::endl;
...
Int_t AtCoreSpecMAT::GetEventID() { return fRawEventPtr->GetEventID(); }
...
void AtCoreSpecMAT::SetAuxChannel(std::vector<Int_t> AuxCh) {
  ...
  if (AuxCh.size() == 0)
    std::cout << cRED << " AtPSAtask : No auxiliary channels found --"
              << cNORMAL << std::endl;
  else {
...
Bool_t AtCoreSpecMAT::GetIsAuxChannel(Int_t val) {
  ...
  return std::find(fAuxChannels.begin(), fAuxChannels.end(), val) !=
         fAuxChannels.end();
}

void AtCoreSpecMAT::SetNumCobo(Int_t numCobo) { fNumCobo = numCobo; }
