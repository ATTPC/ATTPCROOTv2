...
OUTPUT=runFit_${STEP}step_${TOTEV}ev_${RUN}_${SIM}sim.txt
NUMCOM=$(( ($TOTEV/$STEP)-2  )) 
> $OUTPUT

FIRSTBUFF=$(($FIRSTEV+$STEP))

printf "./build/e20009AnaExe $FIRSTEVE $FIRSTBUFF 0 $RUN $DIREC $SIM\n" >> $OUTPUT


for (( i = 0; i <= $NUMCOM; i++ )) 
do    
    SECONDBUFF=$(($FIRSTBUFF+$STEP))
    printf "./build/e20009AnaExe $FIRSTBUFF $SECONDBUFF 0 $RUN $DIREC $SIM\n" >> $OUTPUT
