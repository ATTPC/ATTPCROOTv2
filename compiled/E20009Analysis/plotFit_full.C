...
void plotFit_full(std::string fileFolder = "data_t20_10_20_cov01_elastic/")
{
   ...
   std::system(
      "find ./data_t20_10_20_cov01_elastic -maxdepth 1 -printf \"%f\n\" >test.txt"); // execute the UNIX command "ls -l
   // >test.txt"
         ...
         for (Int_t i = 0; i < nentries; i++) {
            ...
            for (auto index = 0; index < EFitVec->size(); ++index) {
               ...
               if ((*POCAXtrVec)[index] < 100.0) {
                  ...
                  if ((*ziniFitXtrVec)[index] > -10.0 && (*ziniFitXtrVec)[index] < 200.0) {
