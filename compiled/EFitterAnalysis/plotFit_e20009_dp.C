...
void plotFit_e20009_dp(std::string fileFolder = "merged_Be10dp_corr/")
{
   ...
   // Protn gate short
   TCutG *cutp = new TCutG("CUTG", 15);
   cutp->SetVarX("ELossvsBrhoZoom");
   cutp->SetVarY("");
   cutp->SetTitle("Graph");
   cutp->SetFillStyle(1000);
   cutp->SetPoint(0, 164.7082, 0.8507463);
   cutp->SetPoint(1, 521.4015, 0.7192164);
   cutp->SetPoint(2, 962.0225, 0.5009328);
   cutp->SetPoint(3, 1885.229, 0.375);
   cutp->SetPoint(4, 2913.344, 0.2910448);
   cutp->SetPoint(5, 3238.565, 0.2350746);
   cutp->SetPoint(6, 3228.074, 0.1483209);
   cutp->SetPoint(7, 1948.174, 0.1483209);
   cutp->SetPoint(8, 962.0225, 0.2210821);
   cutp->SetPoint(9, 343.0548, 0.3777985);
   cutp->SetPoint(10, 91.27136, 0.5317164);
   cutp->SetPoint(11, 70.2894, 0.8003731);
   cutp->SetPoint(12, 143.7262, 0.8731343);
   cutp->SetPoint(13, 143.7262, 0.8731343);
   cutp->SetPoint(14, 164.7082, 0.8507463);
      ...
      if (!rootfile.IsZombie()) {
         ...
         for (Int_t i = 0; i < nentries; i++) {
            ...
            for (auto index = 0; index < EFitVec->size(); ++index) {
               ...
               if (!cutp->IsInside((*eLossADC)[index], (*brhoVec)[index])) // particleID
                  continue;
               ...
               for (auto iEb = 0; iEb < 100; ++iEb) {
                  auto [Qdep, theta_cm_qdep] =
                     kine_2b(m_Be10, m_d, m_b, m_B, iEb, angle * TMath::DegToRad(), (*EFitVec)[index]);
                  QvsEb->Fill(Qdep, iEb);
               }
   ...
   TCanvas *cKineBuff = new TCanvas("cKineBuff", "cKineBuff", 700, 700);
