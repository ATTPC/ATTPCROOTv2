...
void plotFit_a1957(std::string fileFolder = "data_106_173/")
{
   ...
   Double_t Ebeam_buff = 192.0; // 169.5; // 167.5;
   Double_t m_b;
   Double_t m_B;
         ...
         for (Int_t i = 0; i < nentries; i++) {
            ...
            for (auto index = 0; index < EFitVec->size(); ++index) {
               ...
               auto [ex_energy_exp, theta_cm] =
                  kine_2b(m_C16, m_p, m_b, m_B, Ebeam_buff, angle * TMath::DegToRad(), (*EFitVec)[index]);
               auto [ex_energy_exp_xtr, theta_cm_xtr] =
                  kine_2b(m_C16, m_p, m_b, m_B, Ebeam_buff, angle * TMath::DegToRad(), (*EFitXtrVec)[index]);

               auto [exPRA, thetacmPRA] = kine_2b(m_C16, m_p, m_b, m_B, Ebeam_buff, APRA * TMath::DegToRad(), EPRA);
               ...
               for (auto iEb = 0; iEb < 300; ++iEb) {
                  auto [Qdep, theta_cm_qdep] =
                     kine_2b(m_C16, m_p, m_b, m_B, iEb, angle * TMath::DegToRad(), (*EFitVec)[index]);
                  QvsEb->Fill(Qdep, iEb);
               }
