...
void plotFit_a1954b(std::string fileFolder = "data_143_215/")
{
   ...
   Double_t Ebeam_buff = 150.0; // 169.5; // 167.5;
   Double_t m_b;
   Double_t m_B;
         ...
         for (Int_t i = 0; i < nentries; i++) {
            ...
            for (auto index = 0; index < EFitVec->size(); ++index) {
               ...
               auto [ex_energy_exp, theta_cm] =
                  kine_2b(m_Be12, m_p, m_b, m_B, Ebeam_buff, angle * TMath::DegToRad(), (*EFitVec)[index]);
               auto [ex_energy_exp_xtr, theta_cm_xtr] =
                  kine_2b(m_Be12, m_p, m_b, m_B, Ebeam_buff, angle * TMath::DegToRad(), (*EFitXtrVec)[index]);
