...
namespace std {
   ...
   struct hash<EFieldMapRef> {
    inline size_t operator()(const EFieldMapRef &x) const
    {
      return x._rad + x._zpos * 10000;
    }
   };
}
...
class AtEDistortionModel : public AtSpaceChargeModel {
 ...
 public:
  ...
  void LoadParameters(AtDigiPar *par) override {};
   ...
   Bool_t SetCorrectionMaps(const std::string& zlut,const std::string& radlut,const std::string& tralut); //! Set the mapping files via file path
